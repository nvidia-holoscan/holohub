...
def build_sample(String flowName, boolean submitToCdash, boolean nightlyBuild) {
    stage('Build sample') {
        ...
        withEnv([
            ...
            "CDASH_BUILD_NAME=holoscan-5-0-ea-samples-${cdashPlatformName}-v4l2_depth-${buildNameSuffix}",
                        ...
                        try {
                            sh '''
                                ...
                                python3 ci/report_cdash_test_failure.py \
                                    ...
                                    --test-name 'v4l2_depth.container_build' \
                                    --command './holohub build v4l2_depth' \
...
def test_sample(String flowName, boolean submitToCdash, boolean nightlyBuild) {
    stage('Test sample') {
        def buildNameSuffix = env.gitlabSourceBranch?.trim() ?: 'main'
        def cdashPlatformName = cdash_platform_name(flowName)
        withEnv([
