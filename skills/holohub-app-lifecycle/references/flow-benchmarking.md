...
./holohub run-container <app> <mode> --language <cpp-or-python> \
  ...
  'set -e
   ...
   python benchmarks/holoscan_flow_benchmarking/benchmark.py \
     ...
     --sched greedy -r 3 -i 1 -m <messages> -d <output-dir>'
./holohub run-container <app> <mode> --language <cpp-or-python> \
  --extra-scripts benchmarking --verbose -- \
  'set -e
   ./holohub run <app> <mode> --local --no-local-build \
...
command omits mode and can silently select `default_mode`. Then run `analyze.py`
in the same environment. Check each parser independently; runner and analyzer
short options do not necessarily mean the same thing.
