...
void TensorRTInferenceLogger::log(ILogger::Severity severity, const char* msg) throw() {
  switch (severity) {
    case Severity::kINTERNAL_ERROR: GXF_LOG_ERROR("TRT INTERNAL_ERROR: %s", msg); break;
    case Severity::kERROR: GXF_LOG_ERROR("TRT ERROR: %s", msg); break;
    case Severity::kWARNING: GXF_LOG_WARNING("TRT WARNING: %s", msg); break;
    case Severity::kINFO: GXF_LOG_DEBUG("TRT INFO: %s", msg); break;
    case Severity::kVERBOSE: if (verbose_) { GXF_LOG_DEBUG("TRT VERBOSE: %s", msg); } break;
    default: GXF_LOG_ERROR("TRT UNKNOWN SEVERITY ERROR: %s", msg); break;
  }
}
...
gxf_result_t TapNextInference::registerInterface(gxf::Registrar* registrar) {
  ...
  result &= registrar->parameter(input_tensor_names_init_, "input_tensor_names_init", "Init Input Tensor Names", "Input tensors for Init model.");
  result &= registrar->parameter(input_binding_names_init_, "input_binding_names_init", "Init Input Binding Names", "Input bindings for Init model.");
  result &= registrar->parameter(output_tensor_names_init_, "output_tensor_names_init", "Init Output Tensor Names", "Output tensors for Init model.");
  result &= registrar->parameter(output_binding_names_init_, "output_binding_names_init", "Init Output Binding Names", "Output bindings for Init model.");

  result &= registrar->parameter(input_tensor_names_fwd_, "input_tensor_names_fwd", "Fwd Input Tensor Names", "Input tensors for Fwd model.");
  result &= registrar->parameter(input_binding_names_fwd_, "input_binding_names_fwd", "Fwd Input Binding Names", "Input bindings for Fwd model.");
  result &= registrar->parameter(output_tensor_names_fwd_, "output_tensor_names_fwd", "Fwd Output Tensor Names", "Output tensors for Fwd model.");
  result &= registrar->parameter(output_binding_names_fwd_, "output_binding_names_fwd", "Fwd Output Binding Names", "Output bindings for Fwd model.");

  result &= registrar->parameter(state_tensor_names_, "state_tensor_names", "State Tensor Names", "List of tensor names that are treated as internal states (preserved across steps).");

  result &= registrar->parameter(pool_, "pool", "Pool", "Allocator instance.");

  result &= registrar->parameter(max_workspace_size_, "max_workspace_size", "Max Workspace Size", "", 67108864l);
  ...
  result &= registrar->parameter(relaxed_dimension_check_, "relaxed_dimension_check", "Relaxed Dimension Check", "", true);

  result &= registrar->parameter(grid_size_, "grid_size", "Grid Size", "Grid size for query points", 15);
  result &= registrar->parameter(grid_height_, "grid_height", "Grid Height", "Image height for grid generation", 256);
  result &= registrar->parameter(grid_width_, "grid_width", "Grid Width", "Image width for grid generation", 256);
...
gxf_result_t TapNextInference::start() {
  ...
  if (plugins_lib_namespace && !initLibNvInferPlugins(&cuda_logger_, plugins_lib_namespace.value().c_str())) {
  ...
  if (!static_states_) return gxf::ToResultCode(static_states_);
...
gxf_result_t TapNextInference::setupEngine(EngineContext& ctx,
  ...
  for (int i = 0; i < n_inputs + (int)out_tensors.size(); ++i) {
    ...
    if (!elem_type_exp) return GXF_FAILURE;

    ctx.binding_infos[tensor_name] = BindingInfo{
        binding_index,
...
gxf_result_t TapNextInference::tick() {
  ...
  for (auto& rx : rx_.get()) {
    auto msg = rx->receive();
    if (msg) messages.push_back(std::move(msg.value()));
  }
  ...
  for (size_t i = 0; i < ctx.input_tensor_names.size(); ++i) {
    ...
    bool is_state = false;
    for(const auto& s : state_tensor_names_.get()) {
        if (s == name) { is_state = true; break; }
    }

    if (is_state) {
        auto t = internal_states_.value().get<gxf::Tensor>(name.c_str());
        if (!t) { GXF_LOG_ERROR("Missing internal state %s", name.c_str()); return GXF_FAILURE; }
        ptr = t.value()->pointer();
        shape = t.value()->shape();
    } else if (name == "query_points") {
        auto t = static_states_.value().get<gxf::Tensor>("query_points");
        if (!t) { GXF_LOG_ERROR("Missing query_points"); return GXF_FAILURE; }
        ptr = t.value()->pointer();
        shape = t.value()->shape();
    } else {
    ...
    if (rank_diff > 0) {
      if (verbose_.get()) {
        ...
        for(int d=0; d<static_cast<int32_t>(shape.rank()); ++d) dims.d[d + rank_diff] = shape.dimension(d);
    } else {
  ...
  // EXECUTE
#if NV_TENSORRT_MAJOR < 8 || (NV_TENSORRT_MAJOR == 8 && NV_TENSORRT_MINOR < 5)
  if (!ctx.cuda_execution_ctx->enqueueV2(ctx.cuda_buffers.data(), cuda_stream_handler_.getCudaStream(), nullptr))
#else
    ...
    if (is_state) {
        ...
        if (!maybe_state_t) {
             ...
             if (out_t->size() != state_t->size()) {
                GXF_LOG_ERROR("State size mismatch %s: Output %lu vs State %lu", name.c_str(), out_t->size(), state_t->size());
                return GXF_FAILURE;
             }
        }
  ...
  if (step_tensor) {
    ...
    auto result = out_tensor->reshapeCustom(
      shape,
  ...
  return gxf::ToResultCode(tx_->publish(result_msg.value(), input_timestamp ? input_timestamp->acqtime : 0));
}
...
gxf::Expected<std::string> TapNextInference::queryHostEngineCapability(int dev_id) const {
  ...
  if (cudaGetDeviceProperties(&device_prop, dev_id) != cudaSuccess) return gxf::Unexpected{};
  std::string name = replaceChar(device_prop.name, ' ', '-');
  std::stringstream ss;
  ss << name << "_c" << device_prop.major << device_prop.minor << "_n" << device_prop.multiProcessorCount;
  return ss.str();
}

gxf::Expected<std::string> TapNextInference::findEngineFilePath(const std::string& host_cap, const std::string& model_path) const {
  ...
  // Make unique hash based on model path to avoid collision between init and fwd if they have different names
...
gxf_result_t TapNextInference::createQueryPoints(int32_t batch_size) {
    ...
    for (int b = 0; b < batch_size; ++b) {
        int pt_idx = 0;
        for (int j = 0; j < grid_size; ++j) { // x (width) - Outer loop to match Python meshgrid/flatten order
