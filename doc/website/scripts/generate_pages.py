...
def parse_metadata_file(metadata_file: Path, statistics) -> None:
    ...
    if dest_dir == "applications":
      path = str(metadata_file).removeprefix("/holohub/applications/").removesuffix("/metadata.json")
    ...
    # For operators we put a flat list
      ...
      logger.error(f"Don't know the ouput path for: {dest_dir}")
      return
    ...
    if os.path.exists(readme_path):
      with open(readme_path, 'r') as readme_file:
        ...
        for match in matches:
          ...
          readme_text = readme_text.replace(match,"https://github.com/nvidia-holoscan/holohub/blob/main/"+dest_dir+"/"+relative_path+"/"+imgmatch+"?raw=true")
        ...
        header_text += f":octicons-stack-24: **Minimum Holoscan SDK version:** "+minimum_required_version+"<br>"
        header_text += f":octicons-beaker-24: **Tested Holoscan SDK versions:** "+", ".join(tested_versions)+"<br>"
        header_text += f":octicons-sparkle-fill-24: **Contribution metric:** "+metric_str+"<br>"

        # Add the header text
        if match:
          title=match.group(1)
          title='['+title+'](https://github.com/nvidia-holoscan/holohub/tree/main/'+dest_dir+'/'+path+')'
