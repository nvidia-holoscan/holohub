...
#ifndef HOLOSCAN_OPERATORS_TAPNEXT_INFERENCE_HPP
#define HOLOSCAN_OPERATORS_TAPNEXT_INFERENCE_HPP
...
/**
 * @brief Operator class to perform TapNext inference.
 *
 * This wraps a GXF Codelet(`nvidia::holoscan::tapnext_inference::TapNextInference`).
 */
class TapNextInferenceOp : public holoscan::ops::GXFOperator {
 public:
  HOLOSCAN_OPERATOR_FORWARD_ARGS_SUPER(TapNextInferenceOp, holoscan::ops::GXFOperator)

  TapNextInferenceOp() = default;

  const char* gxf_typename() const override {
    return "nvidia::holoscan::tapnext_inference::TapNextInference";
  }

  void setup(OperatorSpec& spec) override;

 private:
  Parameter<std::string> model_file_path_init_;
  Parameter<std::string> model_file_path_fwd_;
  ...
  Parameter<std::vector<std::string>> output_binding_names_init_;

  Parameter<std::vector<std::string>> input_tensor_names_fwd_;
  ...
  Parameter<int32_t> grid_size_;
  Parameter<int32_t> grid_height_;
  Parameter<int32_t> grid_width_;
