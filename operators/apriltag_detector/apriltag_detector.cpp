...
void ApriltagDetectorOp::start()
{
    ...
    status = nvCreateAprilTagsDetector(&apriltag_handle_,
		    width_,
		    height_,
		    tile_size,
		    NVAT_TAG36H11,
...
void ApriltagDetectorOp::compute(holoscan::InputContext& input,
    ...
    holoscan::ExecutionContext& context)
{
    ...
    if (components != 3) {
        throw std::runtime_error(fmt::format("Unexpected component count {}, expected '3'", components));
    }
    std::vector<cuAprilTagsID_t> tags(number_of_tags_);
    uint32_t num_tags;
    cuAprilTagsImageInput_t input_image =
    {
       reinterpret_cast<uchar3*>(input_tensor->data()),
       width * sizeof(uchar3),
       width,
       height
    };
    ...
    cuAprilTagsDetect(apriltag_handle_, &input_image, tags.data(),
		      &num_tags, number_of_tags_, cuda_stream);
