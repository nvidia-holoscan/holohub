...
from test_utils import assert_frame_properties
...
class GoldenFrameComparator:
    ...
    def compare_frames(self, test_frame, reference_frame):
        ...
        if (
            test_frame.width != reference_frame.width
            or test_frame.height != reference_frame.height
            or test_frame.channels != reference_frame.channels
        ):
            return (
                False,
                1.0,
...
class TestGoldenFrames:
    ...
    def test_golden_frames_directory_exists(self, golden_frames_dir):
        ...
        assert (
            golden_frames_dir.is_dir()
        ), f"Golden frames path is not a directory: {golden_frames_dir}"
    ...
    def test_golden_frames_available(self, golden_frames_dir):
        ...
        ), f"No expected golden frames found. Available: {available_files}"
    ...
    def test_golden_frame_validation(self, golden_frame_comparator, frame_number):
        ...
        assert (
            diff_ratio == 0.0
        ), f"Self-validation should have 0 difference for frame {frame_number}"
    ...
    def test_streaming_server_frame_processing_regression(self, golden_frame_comparator):
        ...
        assert (
            stats["difference_ratio"] <= golden_frame_comparator.tolerance
    ...
    def test_frame_processing_with_mirroring(self, golden_frame_comparator):
        ...
        is_restored, restore_diff, _ = golden_frame_comparator.compare_frames(
            double_mirrored_frame, golden_frame
        )
    ...
    def test_golden_frame_sequence_processing(self, golden_frame_comparator):
        ...
        assert (
            avg_difference <= golden_frame_comparator.tolerance
        ), f"Average difference {avg_difference:.4f} exceeds tolerance {golden_frame_comparator.tolerance}"

        print(
            f"✅ Processed {processed_count} frames with average difference: {avg_difference:.6f}"
        )
