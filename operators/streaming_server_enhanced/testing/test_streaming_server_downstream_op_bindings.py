...
# Test configuration
STREAMING_SERVER_ENHANCED_MOCK_ONLY = os.environ.get('STREAMING_SERVER_ENHANCED_MOCK_ONLY', 'false').lower() == 'true'
...
class TestStreamingServerDownstreamOpBindings:
    ...
    def test_streaming_server_downstream_op_with_resource(self):
        ...
        try:
            ...
            pytest.skip(f"StreamingServerDownstreamOp or StreamingServerResource not available: {e}")
        ...
        try:
            ...
            for width, height, fps, mirror, description in processing_configs:
                try:
                    ...
                    if hasattr(downstream_op, 'width') and callable(getattr(downstream_op, 'width')):
                        downstream_op.width(width)
                    if hasattr(downstream_op, 'height') and callable(getattr(downstream_op, 'height')):
                        downstream_op.height(height)
                    if hasattr(downstream_op, 'fps') and callable(getattr(downstream_op, 'fps')):
                        downstream_op.fps(fps)
                    if hasattr(downstream_op, 'mirror') and callable(getattr(downstream_op, 'mirror')):
    ...
    def test_streaming_server_downstream_op_error_handling(self):
        ...
        try:
            ...
            for param_name, error_values, description in error_tests:
                if hasattr(downstream_op, param_name) and callable(getattr(downstream_op, param_name)):
                    ...
                    for value in error_values:
                        try:
                            ...
                            print(f"✅ {param_name}({value}) correctly rejected: {type(e).__name__}")
                else:
    ...
    def test_streaming_server_downstream_op_resource_consistency(self):
        ...
        try:
            ...
            if (hasattr(downstream_op, 'streaming_server_resource') and 
                hasattr(upstream_op, 'streaming_server_resource')):
                ...
                try:
                    ...
                    if callable(getattr(downstream_op, 'streaming_server_resource')):
                        ...
                        print("✅ Successfully shared resource between upstream and downstream operators")
                except Exception as e:
...
@pytest.mark.bindings
@pytest.mark.skipif(not STREAMING_SERVER_ENHANCED_MOCK_ONLY, reason="Only run fallback test in mock-only mode")
class TestStreamingServerDownstreamOpBindingsFallback:
    ...
    def test_binding_availability_info(self):
        ...
        try:
            ...
            print("ℹ️ StreamingServerDownstreamOp bindings not available - this is expected in mock-only mode")
            print("ℹ️ To test real bindings, build with streaming server libraries and set BUILD_TESTING=ON")
