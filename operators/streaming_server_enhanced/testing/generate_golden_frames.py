    ...
    if pattern_type == "gradient":
        ...
        for y in range(height):
            for x in range(width):
                ...
                frame[y, x, 2] = min(255, (((x + y) + offset) * 255) // (width + height))  # Blue channel
        ...
        frame[text_y:text_y+text_height, text_start_x:text_start_x+text_width] = [255, 255, 255]
        ...
        frame = np.zeros((height, width, 3), dtype=np.uint8)
        border_width = 20
        border_color = [(frame_number * 30) % 256, (frame_number * 50) % 256, (frame_number * 70) % 256]
...
def generate_golden_frames(output_dir, width=854, height=480, count=10, patterns=None):
    ...
    with open(metadata_file, 'w') as f:
        f.write(f"Golden Frames Metadata\n")
        f.write(f"======================\n")
        f.write(f"Generated frames: {generated_count}\n")
        f.write(f"Resolution: {width}x{height}\n")
        f.write(f"Patterns used: {', '.join(patterns)}\n")
        f.write(f"Frame format: PNG (RGB)\n")
        f.write(f"Usage: Visual regression testing for StreamingServer\n")
        f.write(f"\nFrame Details:\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate golden reference frames for StreamingServer testing")
    
    parser.add_argument("--output-dir", "-o", 
                       default="golden_frames",
                       help="Output directory for golden frames (default: golden_frames)")
    
    parser.add_argument("--width", "-w", type=int, default=854,
