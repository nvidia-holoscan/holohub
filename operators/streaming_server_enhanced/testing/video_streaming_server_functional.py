...
from mock_holoscan_framework import MockFrame, create_mock_bgr_frame
...
class VideoFrameValidatorOp(Operator):
        ...
        if tensor_data:
            ...
            try:
                ...
                if shape is None or len(shape) != 3:
                    self.validation_errors.append(f"Frame {self.frames_received}: Invalid shape {shape}")
                    return
                ...
                if channels != 3:
                    self.validation_errors.append(f"Frame {self.frames_received}: Wrong channel count {channels}")
                    return
                
                logger.debug(f"Validated frame {self.frames_received}: {width}x{height}x{channels}")
                
            except Exception as e:
                self.validation_errors.append(f"Frame {self.frames_received}: Validation error: {e}")
    ...
    def get_validation_results(self):
        ...
        return {
            ...
            "success_rate": (self.frames_received - len(self.validation_errors)) / max(1, self.frames_received)
        }
...
class StreamingServerFunctionalTestApp(Application):
    ...
    def _setup_mock_pipeline(self):
        ...
        server_resource = MockStreamingServerResource(mock_server, {
            "width": 854,
            "height": 480,
            "fps": 30,
            "port": 48010
        ...
        video_generator = VideoFrameGeneratorOp(
            self, name="video_generator",
            frame_count=10, width=854, height=480, fps=30
        )
        
        upstream_op = MockStreamingServerUpstreamOp(
            self, name="streaming_server_upstream",
            width=854, height=480, fps=30,
            streaming_server_resource=server_resource
        )
        
        downstream_op = MockStreamingServerDownstreamOp(
            self, name="streaming_server_downstream", 
            width=854, height=480, fps=30,
            enable_processing=True, processing_type="mirror",
            streaming_server_resource=server_resource
        )
        
        validator = VideoFrameValidatorOp(
            self, name="video_validator",
...
def main():
    ...
    parser.add_argument("--minimal", action="store_true",
                       help="Run minimal infrastructure test only")
    ...
    try:
        ...
        app = StreamingServerFunctionalTestApp(
