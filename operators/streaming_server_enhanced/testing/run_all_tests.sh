...
run_unit_tests() {
    ...
    for test_file in "${unit_test_files[@]}"; do
        if [[ -f "${SCRIPT_DIR}/$test_file" ]]; then
            ...
            # Use exact same approach as working individual tests  
            if timeout "$timeout" bash -c "python3 -m pip install --user pytest --quiet && python3 -m pytest ${pytest_args[*]} \"$SCRIPT_DIR/$test_file\"" >> "$LOG_FILE" 2>&1; then
...
run_golden_frame_tests() {
    ...
    # Use exact same approach as working individual tests
    if timeout "$timeout" bash -c "python3 -m pip install --user pytest --quiet && python3 -m pytest ${pytest_args[*]} \"$golden_test_file\"" >> "$LOG_FILE" 2>&1; then
