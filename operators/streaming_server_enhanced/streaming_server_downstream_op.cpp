...
void StreamingServerDownstreamOp::compute(InputContext& op_input, OutputContext& op_output,
  ...
  try {
    ...
    if (output_frame.getDataSize() > 0) {
      ...
      HOLOSCAN_LOG_ERROR("❌ DOWNSTREAM: Cannot send frame - "
                          "invalid data size: {}",
                          output_frame.getDataSize());
    }
    ...
    if (frames_processed_ % 30 == 0) {
      ...
      if (elapsed.count() > 0) {
        ...
        HOLOSCAN_LOG_INFO("📊 Downstream Performance: "
                            "Processed {} frames, Sent {} frames ({:.2f} FPS)",
...
Frame StreamingServerDownstreamOp::convert_tensor_to_frame(const holoscan::Tensor& tensor) {
  ...
  if (shape.size() != 3) {
    throw std::runtime_error(fmt::format("Expected 3D tensor in HWC format (height, "
                        "width, channels), but got {}D tensor with shape [{}]",
  ...
  if (height == 0 || width == 0 || channels == 0) {
    throw std::runtime_error(fmt::format("Invalid tensor dimensions: "
  ...
  if (dtype.code == kDLUInt && dtype.bits == 8 && dtype.lanes == 1) {  // uint8
    ...
    throw std::runtime_error(fmt::format("Unsupported tensor data type: "
                        "code={}, bits={}, lanes={}. Only uint8 tensors are supported.", dtype.code, dtype.bits, dtype.lanes));
  }
