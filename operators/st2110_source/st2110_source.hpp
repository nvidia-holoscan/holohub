 ...
 *
 * This implementation uses standard Linux sockets (no DPDK required), following the
 * pattern used by NVIDIA's Holoscan Sensor Bridge for efficient CPU-to-GPU transfer.
 *
 * ==Named Outputs==
 *
...
class ST2110SourceOp : public holoscan::Operator {
 ...
 private:
  ...
  struct FrameBuffer {
    void* pinned_memory = nullptr;         // CUDA pinned memory (CU_MEMHOSTALLOC_WRITECOMBINED)
    uint32_t current_timestamp = 0;        // RTP timestamp of current frame
    uint32_t packets_received = 0;         // Packets received for current frame
    uint32_t bytes_received = 0;           // Bytes received for current frame
    std::atomic<bool> frame_complete{false};  // Frame reassembly complete
  };
