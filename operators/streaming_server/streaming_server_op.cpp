...
void StreamingServerOp::setup(OperatorSpec& spec) {
 ...
 "Memory allocator for frame data");
  ...
  HOLOSCAN_LOG_INFO("StreamingServerOp setup completed - standalone mode "
...
void StreamingServerOp::initialize() {
  ...
  HOLOSCAN_LOG_INFO("  width: {}",
                    width_.has_value() ? std::to_string(width_.get()) : "NOT SET");
  HOLOSCAN_LOG_INFO("  height: {}",
                    height_.has_value() ? std::to_string(height_.get()) :
                    "NOT SET");
  HOLOSCAN_LOG_INFO("  fps: {}",
                    fps_.has_value() ? std::to_string(fps_.get()) : "NOT SET");
  HOLOSCAN_LOG_INFO("  port: {}",
                    port_.has_value() ? std::to_string(port_.get()) : "NOT SET");
  HOLOSCAN_LOG_INFO("  receive_frames: {}",
                    receive_frames_.has_value() ? 
                    (receive_frames_.get() ? "true" : "false") : "NOT SET");
  HOLOSCAN_LOG_INFO("  send_frames: {}",
                    send_frames_.has_value() ? 
                    (send_frames_.get() ? "true" : "false") : "NOT SET");
...
port_ = static_cast<uint16_t>(48010);
  ...
  config.serverName = server_name_.has_value() ? server_name_.get() : 
                      "StreamingServer";

  HOLOSCAN_LOG_INFO("Creating StreamingServer with validated config: "
