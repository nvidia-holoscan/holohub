...
cp lib/$(uname -m)/*.so* lib/
cp -r lib/$(uname -m)/plugins lib/
...
The streaming server tests now automatically download surgical video data at build time for realistic testing:
