...
class GoldenFrameComparator:
    ...
    def compare_frames(self, actual_frame: np.ndarray, golden_frame: np.ndarray) -> Tuple[bool, float, dict]:
        """
    ...
    def generate_diff_visualization(self, actual_frame: np.ndarray, golden_frame: np.ndarray, 
        ...
        diff_normalized = (diff / np.max(diff) * 255).astype(np.uint8) if np.max(diff) > 0 else diff.astype(np.uint8)
...
class TestGoldenFrames:
    ...
    def test_mock_frame_against_golden(self, golden_frame_comparator, frame_number):
        ...
        assert diff_ratio <= golden_frame_comparator.tolerance, \
...
def create_test_frame_from_golden(golden_frames_dir: str, frame_number: int, 
...
def run_golden_frame_validation(output_frames: List[np.ndarray], 
                               golden_frames_dir: str,
