...
def streaming_client_op_class(holoscan_modules):
    ...
    try:
        ...
        try:
            ...
            pytest.skip(
                f"Cannot import StreamingClientOp: {e}. Make sure the operator is built with Python bindings."
            )
...
def operator_factory(mock_fragment, streaming_client_op_class, default_operator_params):
    ...
    def _create_operator(**kwargs):
        ...
        return streaming_client_op_class(mock_fragment, **params)
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.getoption("--skip-hardware-tests"):
        skip_hardware = pytest.mark.skip(
            reason="Skipping hardware tests (--skip-hardware-tests option)"
        )
        for item in items:
