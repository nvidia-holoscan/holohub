...
DocaRxQueue::DocaRxQueue(struct doca_dev *dev_, struct doca_gpu *gdev_, struct doca_flow_port *df_port_, uint16_t qid_, int max_pkt_num_, int max_pkt_size_)
			:
			ddev(dev_), gdev(gdev_), df_port(df_port_), qid(qid_),
			max_pkt_num(max_pkt_num_), max_pkt_size(max_pkt_size_)
{
	HOLOSCAN_LOG_INFO("Creating UDP Eth Rxq {} max_pkt_size {} max_pkt_num {}", qid, max_pkt_size, max_pkt_num);
	...
	result = doca_eth_rxq_estimate_packet_buf_size(DOCA_ETH_RXQ_TYPE_CYCLIC,
							...
							&cyclic_buffer_size);
	if (result != DOCA_SUCCESS) {
		HOLOSCAN_LOG_CRITICAL("Failed to get eth_rxq cyclic buffer size: {}", doca_error_get_descr(result));
	}
	...
	result = doca_gpu_mem_alloc(gdev, cyclic_buffer_size, GPU_PAGE_SIZE, DOCA_GPU_MEM_TYPE_GPU, &gpu_pkt_addr, nullptr);
	...
	// if (result != DOCA_SUCCESS) {
		HOLOSCAN_LOG_INFO("Mapping receive queue buffer (0x{} size {}B) with nvidia-peermem mode",
			gpu_pkt_addr, cyclic_buffer_size);
...
/*
		...
		HOLOSCAN_LOG_INFO("Mapping receive queue buffer (0x{} size {}B dmabuf fd {}) with dmabuf mode",
			gpu_pkt_addr, cyclic_buffer_size, dmabuf_fd);

		result = doca_mmap_set_dmabuf_memrange(pkt_buff_mmap, dmabuf_fd, gpu_pkt_addr, 0, cyclic_buffer_size);
		if (result != DOCA_SUCCESS) {
			HOLOSCAN_LOG_CRITICAL("Failed to set dmabuf memrange for mmap {}", doca_error_get_descr(result));
		}
	}
*/
	result = doca_mmap_set_permissions(pkt_buff_mmap, DOCA_ACCESS_FLAG_LOCAL_READ_WRITE | DOCA_ACCESS_FLAG_PCI_RELAXED_ORDERING);
	if (result != DOCA_SUCCESS) {
...
doca_error_t
DocaRxQueue::create_udp_pipe(const FlowConfig &cfg, struct doca_flow_pipe *rxq_pipe_default)
{
	doca_error_t result;
	...
	HOLOSCAN_LOG_INFO("UDP pipe with src port {} dst port {}", cfg.match_.udp_src_, cfg.match_.udp_dst_);
	...
	/* Add HW offload */
	result = doca_flow_pipe_add_entry(0, rxq_pipe, &match, nullptr, nullptr, nullptr, DOCA_FLOW_NO_WAIT, nullptr, &entry);
	if (result != DOCA_SUCCESS) {
...
DocaRxQueue::create_semaphore()
{
	...
	*/
	...
	if (result != DOCA_SUCCESS) {
		HOLOSCAN_LOG_ERROR("Failed doca_gpu_semaphore_set_memory_type: {}", doca_error_get_descr(result));
		return DOCA_ERROR_BAD_STATE;
	}
	...
	*/
	result = doca_gpu_semaphore_set_custom_info(sem_cpu, sizeof(struct adv_doca_rx_gpu_info), DOCA_GPU_MEM_TYPE_CPU_GPU);
	if (result != DOCA_SUCCESS) {
		HOLOSCAN_LOG_ERROR("Failed doca_gpu_semaphore_set_custom_info: {}", doca_error_get_descr(result));
		return DOCA_ERROR_BAD_STATE;
	}
	...
	if (result != DOCA_SUCCESS) {
		HOLOSCAN_LOG_ERROR("Failed doca_gpu_semaphore_get_gpu_handle: {}", doca_error_get_descr(result));
		return DOCA_ERROR_BAD_STATE;
	}
...
DocaTxQueue::DocaTxQueue(struct doca_dev *ddev_, struct doca_gpu *gdev_, uint16_t qid_, int max_pkt_num_, int max_pkt_size_, doca_eth_txq_gpu_event_notify_send_packet_cb_t event_notify_send_packet_cb)
			:
			ddev(ddev_), gdev(gdev_), qid(qid_),
			max_pkt_num(max_pkt_num_), max_pkt_size(max_pkt_size_)
{
	doca_error_t result;
	...
	fprintf(stderr, "tx_buffer_size %d max_pkt_size %d max_pkt_num %d\n", tx_buffer_size, max_pkt_size, max_pkt_num);
	...
	result = doca_eth_txq_gpu_event_notify_send_packet_register(eth_txq_cpu,
	...
	result = doca_gpu_mem_alloc(gdev, tx_buffer_size, GPU_PAGE_SIZE, DOCA_GPU_MEM_TYPE_GPU, &gpu_pkt_addr, nullptr);
	if (result != DOCA_SUCCESS || gpu_pkt_addr == nullptr) {
...
/*
		...
		HOLOSCAN_LOG_INFO("Mapping receive queue buffer (0x{} size {}B dmabuf fd {}) with dmabuf mode",
			gpu_pkt_addr, tx_buffer_size, dmabuf_fd);

		result = doca_mmap_set_dmabuf_memrange(pkt_buff_mmap, dmabuf_fd, gpu_pkt_addr, 0, tx_buffer_size);
		if (result != DOCA_SUCCESS) {
