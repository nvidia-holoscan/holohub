...
doca_error_t DocaMgr::init_doca_devices() {
  ...
  for (const auto &intf: cfg_.ifs_) {
    ...
    HOLOSCAN_LOG_INFO("DOCA init Port {} -- RX: {} TX: {}", intf.port_id_,
...
struct doca_flow_port* DocaMgr::init_doca_flow(uint16_t port_id, uint8_t rxq_num) {
  ...
  for (int idx = 0; idx < rxq_num; idx++) {
    ...
    std::string name = std::string("RX_POOL_P") + std::to_string(port_id) + "_Q" + std::to_string(idx);
  ...
  if (!flow_init) {
    ...
    if (ret != DOCA_SUCCESS) {
      HOLOSCAN_LOG_CRITICAL("Failed to set doca_flow_cfg pipe_queues: {}", doca_error_get_descr(ret));
    ...
    */
    ...
    if (ret != DOCA_SUCCESS) {
      HOLOSCAN_LOG_CRITICAL("Failed to set doca_flow_cfg nr_counters: {}", doca_error_get_descr(ret));
      doca_flow_cfg_destroy(rxq_flow_cfg);
      return nullptr;
    }
    ...
    if (result != DOCA_SUCCESS) {
      HOLOSCAN_LOG_CRITICAL("Failed to init doca flow with: {}:{}", (int)result, doca_error_get_descr(result));
      return nullptr;
    }
...
void DocaMgr::initialize() {
  ...
  for (auto &intf: cfg_.ifs_) {
    for (auto& q : intf.rx_.queues_) {
      HOLOSCAN_LOG_INFO(
          "Configuring RX queue: {} ({}) on port {}", q.common_.name_, q.common_.id_, intf.port_id_);
      const auto mr_name = q.common_.mrs_[0]; // Only use the first MR (no splitting)
      ...
      HOLOSCAN_LOG_INFO(
          ...
          q.common_.name_, q.common_.id_, intf.port_id_, mtype, rxq_pkts, q_max_packet_size);

      rx_q_map_[key] = new DocaRxQueue(
          ddev[intf.port_id_], gdev[gpu_id], df_port[intf.port_id_], q.common_.id_, rxq_pkts, q_max_packet_size, mtype);
    }
