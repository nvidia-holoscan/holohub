...
static doca_error_t
init_doca_device(char **argv, int argc, const char *nic_pcie_addr, struct doca_dev **ddev, uint16_t *dpdk_port_id)
{
    doca_error_t result;
    int ret;
    ...
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_CRITICAL("Function doca_dpdk_port_probe returned %s", doca_error_get_descr(result));
        return result;
    }

    result = doca_dpdk_get_first_port_id(*ddev, dpdk_port_id);
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_CRITICAL("Function doca_dpdk_get_first_port_id returned %s", doca_error_get_descr(result));
        return result;
    }
...
DocaMgr::init_doca_flow(uint16_t port_id, uint8_t rxq_num)
{
    doca_error_t result;
    ...
    for (int idx = 0; idx < rxq_num; idx++) {
        ret_dpdk = rte_eth_rx_queue_setup(port_id, idx, 2048, rte_eth_dev_socket_id(port_id), nullptr, mp);
        if (ret) {
    ...
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_CRITICAL("Failed to set doca_flow_port_cfg devargs: %s", doca_error_get_descr(result));
...
int DocaMgr::setup_pools_and_rings(int max_rx_batch, int max_tx_batch) {
    ...
    rx_ring = rte_ring_create("RX_RING", 2048, rte_socket_id(), RING_F_MC_RTS_DEQ | RING_F_MP_RTS_ENQ);
    if (rx_ring == nullptr) {
    ...
    rx_meta = rte_mempool_create("RX_META_POOL", (1U << 6) - 1U, sizeof(AdvNetBurstParams),
                    0, 0, nullptr, nullptr, nullptr, nullptr, rte_socket_id(), 0);
    if (rx_meta == nullptr) {
        HOLOSCAN_LOG_CRITICAL("Failed to allocate RX meta pool!");
        return -1;
    }

    while (idx < (1U << 6) - 1U && rte_mempool_get(rx_meta, reinterpret_cast<void **>(&bursts_rx[idx])) == 0) {
    ...
    for (const auto &tx : cfg_.tx_) {
        for (const auto &q : tx.queues_) {
            ...
            tx_rings[key] = rte_ring_create(name.c_str(), 2048, rte_socket_id(), RING_F_MC_RTS_DEQ | RING_F_MP_RTS_ENQ);
    ...
    idx = 0;
    while (idx < (1U << 7) - 1U && rte_mempool_get(tx_meta, reinterpret_cast<void **>(&bursts_tx[idx])) == 0) {
...
void DocaMgr::initialize() {
    ...
    for (int i = 0; i < max_nargs; i++) {
        argv_[i] = (char*)malloc(max_arg_size);
    }
    ...
    if (cfg_.rx_.size() > 0) {
        ...
        doca_ret = init_doca_device(argv_, arg, cfg_.rx_[0].if_name_.c_str(), &ddev_rx, &cfg_.rx_[0].port_id_);
        ...
        for (auto &rx : cfg_.rx_)
            rx.port_id_ = cfg_.rx_[0].port_id_;
        
        if (cudaDeviceGetPCIBusId(gpu_bdf, sizeof(gpu_bdf), cfg_.rx_[0].queues_[0].common_.gpu_dev_) != cudaSuccess) {
            HOLOSCAN_LOG_CRITICAL("Failed get GPU PCIe addr device {}", cfg_.rx_[0].queues_[0].common_.gpu_dev_);
            return;
        }
    }
    ...
    if (cfg_.tx_.size() > 0) {
        ...
        if (
            ...
            doca_ret = init_doca_device(argv_, arg, cfg_.tx_[0].if_name_.c_str(), &ddev_tx, &cfg_.tx_[0].port_id_);
            if (doca_ret != DOCA_SUCCESS) {
                HOLOSCAN_LOG_CRITICAL("Failed init DOCA device {}", net_bdf);
                return;
            }
            ...
            for (auto &tx : cfg_.tx_)
                tx.port_id_ = cfg_.tx_[0].port_id_;
        } else {
             for (auto &tx : cfg_.tx_)
                tx.port_id_ = cfg_.rx_[0].port_id_;
        }

        // Need to create 2 GPU devices in case of RX + TX support!!!
        if (cudaDeviceGetPCIBusId(gpu_bdf, sizeof(gpu_bdf), cfg_.tx_[0].queues_[0].common_.gpu_dev_) != cudaSuccess) {
            HOLOSCAN_LOG_CRITICAL("Failed get GPU PCIe addr device {}", cfg_.tx_[0].queues_[0].common_.gpu_dev_);
            return;
        }
    }
    ...
    HOLOSCAN_LOG_INFO("DOCA init -- RX: {} TX: {}", cfg_.rx_.size() > 0 ? "ENABLED" : "DISABLED", cfg_.tx_.size() > 0 ? "ENABLED" : "DISABLED");
    ...
    for (auto &rx : cfg_.rx_) {
        for (auto &q : rx.queues_) {
            ...
            q.common_.backend_config_ = new DocaRxQueue(ddev_rx, gdev, df_port, q.common_.id_, rxq_pkts, q.common_.max_packet_size_);
        }
    }
    ...
    for (auto &tx : cfg_.tx_) {
        for (auto &q : tx.queues_) {
            HOLOSCAN_LOG_INFO("Configuring TX queue: {} ({}) on port {}", 
                        q.common_.name_, q.common_.id_, tx.port_id_);

            txq_pkts = next_power_of_two(q.common_.num_concurrent_batches_) * next_power_of_two((uint64_t)q.common_.batch_size_);
            q.common_.backend_config_ = new DocaTxQueue(ddev_tx, gdev, q.common_.id_, txq_pkts, q.common_.max_packet_size_, &decrease_txq_completion_cb);
        }
    }
...
doca_error_t
DocaMgr::create_default_pipe(uint32_t cnt_defq)
{
    ...
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_ERROR("Failed to set doca_flow_pipe_cfg is_root: %s", doca_error_get_descr(result));
        return result;
    }
    ...
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_ERROR("Failed to set doca_flow_pipe_cfg monitor: %s", doca_error_get_descr(result));
        return result;
    }
    ...
    /* Add HW offload */
    result = doca_flow_pipe_add_entry(0, rxq_pipe_default, &match, nullptr, nullptr, nullptr, DOCA_FLOW_NO_WAIT, nullptr, &entry);
    if (result != DOCA_SUCCESS) {
...
DocaMgr::create_root_pipe()
{
    ...
    for (const auto &rx : cfg_.rx_) {
        for (const auto &flow : rx.flows_) {
            ...
            result = doca_flow_pipe_control_add_entry(0, 0, root_pipe, 
                                    &udp_match,
                                    nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                                    &udp_fwd, nullptr, &(q_backend->root_udp_entry));
    ...
    if (cnt_defq > 0) {
        ...
        result = doca_flow_pipe_control_add_entry(0, 1, root_pipe, &udp_match,
                                nullptr, nullptr, nullptr, nullptr, nullptr, nullptr,
                                &udp_fwd, nullptr, &(root_udp_entry_default));
        if (result != DOCA_SUCCESS) {
            HOLOSCAN_LOG_CRITICAL("Root pipe UDP entry creation failed with: {}",
                        doca_error_get_descr(result));
            return result;
...
DocaMgr::~DocaMgr() {
    for (auto &rx : cfg_.rx_) {
        ...
        /* Tx burst preallocate */
    for (int idx = 0; idx < MAX_TX_BURST; idx++)
        cudaFree(burst[idx].gpu_pkts_len);
}
...
int DocaMgr::rx_core(void *arg) {
    ...
    HOLOSCAN_LOG_INFO("Starting RX Core {}, port {}, queues {}, socket {}",
        rte_lcore_id(), tparams->port, tparams->rxqn, rte_socket_id());
    ...
    */
    ...
    result = doca_gpu_mem_alloc(tparams->gdev, tparams->rxqn * sizeof(uintptr_t), GPU_PAGE_SIZE,
                                DOCA_GPU_MEM_TYPE_CPU_GPU, (void**)&eth_rxq_gpu_list, (void**)&eth_rxq_cpu_list);
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_ERROR("Failed to allocate gpu memory before launching kernel {}", doca_error_get_descr(result));
        exit(1);
    }

    result = doca_gpu_mem_alloc(tparams->gdev, tparams->rxqn * sizeof(uintptr_t), GPU_PAGE_SIZE,
                                DOCA_GPU_MEM_TYPE_CPU_GPU, (void**)&sem_gpu_list, (void**)&sem_cpu_list);
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_ERROR("Failed to allocate gpu memory before launching kernel {}", doca_error_get_descr(result));
        exit(1);
    }
    ...
    result = doca_gpu_mem_alloc(tparams->gdev, tparams->rxqn * sizeof(uintptr_t), GPU_PAGE_SIZE,
                                DOCA_GPU_MEM_TYPE_CPU_GPU, (void**)&batch_gpu_list, (void**)&batch_cpu_list);
    if (result != DOCA_SUCCESS) {
        HOLOSCAN_LOG_ERROR("Failed to allocate gpu memory before launching kernel {}", doca_error_get_descr(result));
        exit(1);
    }
    ...
    result = doca_gpu_mem_alloc(tparams->gdev, sizeof(uint32_t), 4096, DOCA_GPU_MEM_TYPE_GPU_CPU, (void **)&gpu_exit_condition, (void **)&cpu_exit_condition);
    ...
    doca_receiver_packet_kernel(rx_stream, tparams->rxqn, nullptr, sem_gpu_list, batch_cpu_list, gpu_exit_condition);
    DOCA_GPUNETIO_VOLATILE(*cpu_exit_condition) = 1;
    cudaStreamSynchronize(rx_stream);

    HOLOSCAN_LOG_INFO("Launch receive kernel");
    DOCA_GPUNETIO_VOLATILE(*cpu_exit_condition) = 0;
    doca_receiver_packet_kernel(rx_stream, tparams->rxqn, eth_rxq_gpu_list, sem_gpu_list, batch_cpu_list, gpu_exit_condition);

    while (!force_quit_doca.load()) {
        for (int ridx = 0; ridx < tparams->rxqn; ridx++) {
            // HOLOSCAN_LOG_INFO("Check queue {} sem {}", ridx, sem_idx[ridx]);
            result = doca_gpu_semaphore_get_status(tparams->rxqw[ridx].rxq->sem_cpu, sem_idx[ridx], &status);
            if (result != DOCA_SUCCESS) {
            ...
            if (status == DOCA_GPU_SEMAPHORE_STATUS_READY) {
                result = doca_gpu_semaphore_get_custom_info_addr(tparams->rxqw[ridx].rxq->sem_cpu, sem_idx[ridx], (void **)&(packets_stats));
                if (result != DOCA_SUCCESS) {
                ...
                // HOLOSCAN_LOG_DEBUG("sem {} queue {} num_pkts {}", sem_idx[ridx], ridx, burst->hdr.hdr.num_pkts);
                // Assuming each batch is accumulated by the kernel
                rte_ring_enqueue(tparams->ring, reinterpret_cast<void *>(burst));

                result = doca_gpu_semaphore_set_status(tparams->rxqw[ridx].rxq->sem_cpu, sem_idx[ridx], DOCA_GPU_SEMAPHORE_STATUS_FREE);
                if (result != DOCA_SUCCESS) {
                    HOLOSCAN_LOG_ERROR("UDP semaphore error queue {}", ridx);
                    force_quit_doca.store(true);
                    break;
    ...
    for (int ridx = 0; ridx < tparams->rxqn; ridx++) {
        ...
        if (status == DOCA_GPU_SEMAPHORE_STATUS_READY) {
            doca_gpu_semaphore_get_custom_info_addr(tparams->rxqw[ridx].rxq->sem_cpu, sem_idx[ridx], (void **)&(packets_stats));
    ...
    HOLOSCAN_LOG_INFO("Total packets received by application (port/queue {}/{}): {}, last partial batch packets {}\n",
        tparams->port, tparams->rxqn, total_pkts+last_batch, last_batch);
...
int DocaMgr::tx_core(void *arg) {
    ...
    HOLOSCAN_LOG_INFO("Starting Tx Core {}, port {}, queues {}, socket {}",
        rte_lcore_id(), tparams->port, tparams->txqn, rte_socket_id());
    ...
    */
    ...
    for (int idxq = 0; idxq < tparams->txqn; idxq++) {
        ...
        doca_sender_packet_kernel(tx_stream[idxq], tparams->txqw[idxq].txq->eth_txq_gpu,
    ...
    while (!force_quit_doca.load()) {
        for (int idxq = 0; idxq < tparams->txqn; idxq++) {
            /* Guardrail to prevent issues caused on ARM by the communication between application and operator */
            if (tparams->txqw[idxq].txq->tx_cmp_posted.load() > TX_COMP_THRS) {
                HOLOSCAN_LOG_DEBUG("Queue {} pkts {} too many cmp {}",
                                    idxq, cnt_pkts[idxq], tparams->txqw[idxq].txq->tx_cmp_posted.load());
                continue;
            }
...
void *DocaMgr::get_gpu_pkt_ptr(AdvNetBurstParams *burst, int idx)   {
    ...
    if (pkt < burst->hdr.hdr.max_pkt)
        ...
        return (void*) (((uintptr_t)burst->hdr.hdr.first_pkt_addr) + ((pkt % burst->hdr.hdr.max_pkt) * burst->hdr.hdr.max_pkt_size));
}
    ...
    for (auto &tx : cfg_.tx_) {
        for (auto &q : tx.queues_) {
            if (q.common_.id_ == burst->hdr.hdr.q_id) {
                ...
                burst->hdr.hdr.gpu_pkt0_addr = (uintptr_t)((uint8_t*)txq->gpu_pkt_addr + (buf_idx * txq->max_pkt_size));
                burst->hdr.hdr.first_pkt_addr = (uintptr_t) txq->gpu_pkt_addr;
                burst->hdr.hdr.gpu_pkt0_idx = buf_idx;

                HOLOSCAN_LOG_DEBUG("Get TX burst for queue {} ({}) on port {} pkts {} first {} gpu_pkt0_idx {}", 
                        q.common_.name_, q.common_.id_, tx.port_id_, burst->hdr.hdr.num_pkts,
                        burst->hdr.hdr.first_pkt_addr, burst->hdr.hdr.gpu_pkt0_idx);
            }
        }
    }
...
AdvNetStatus DocaMgr::set_cpu_eth_hdr(AdvNetBurstParams *burst,
                                      int idx,
                                      uint8_t *dst_addr) {
  return AdvNetStatus::NOT_SUPPORTED;
}

AdvNetStatus DocaMgr::set_cpu_ipv4_hdr(AdvNetBurstParams *burst,
                                      int idx,
                                      int ip_len,
                                      uint8_t proto,
...
AdvNetStatus DocaMgr::set_cpu_udp_hdr(AdvNetBurstParams *burst,
                                      int idx,
                                      int udp_len,
                                      uint16_t src_port,
                                      uint16_t dst_port) {
...
AdvNetBurstParams * DocaMgr::create_burst_params() {
    ...
    HOLOSCAN_LOG_DEBUG("create_burst_params burst_idx {} MAX_TX_BURST {}",
