...
class DocaRxQueue {
public:
	DocaRxQueue(struct doca_dev *dev, struct doca_gpu *gdev, struct doca_flow_port *df_port, uint16_t qid, int max_pkt_num, int max_pkt_size);
	~DocaRxQueue();
...
class DocaTxQueue {
public:
	DocaTxQueue(struct doca_dev *dev, struct doca_gpu *gdev, uint16_t qid, int max_pkt_num, int max_pkt_size, doca_eth_txq_gpu_event_notify_send_packet_cb_t event_notify_send_packet_cb);
	~DocaTxQueue();
