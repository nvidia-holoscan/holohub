 ...
 */
__global__ void receive_packets_kernel(int rxqn, uintptr_t *eth_rxq_gpu, uintptr_t *sem_gpu, const uint32_t *batch_list, uint32_t *exit_cond)
{
	doca_error_t ret;
	...
	while (DOCA_GPUNETIO_VOLATILE(*exit_cond) == 0) {
		ret = doca_gpu_dev_eth_rxq_receive_block(rxq, CUDA_MAX_RX_NUM_PKTS, CUDA_MAX_RX_TIMEOUT_NS, &rx_pkt_num, &rx_buf_idx);
		...
		while (buf_idx < rx_pkt_num && buf_idx < CUDA_MAX_RX_NUM_PKTS) {
			...
			printf("Queue %d Thread %d received UDP packet with Eth src %02x:%02x:%02x:%02x:%02x:%02x - Eth dst %02x:%02x:%02x:%02x:%02x:%02x - Src port %d - Dst port %d\n",
				blockIdx.x, threadIdx.x,
				((uint8_t *)hdr->l2_hdr.s_addr_bytes)[0], ((uint8_t *)hdr->l2_hdr.s_addr_bytes)[1], ((uint8_t *)hdr->l2_hdr.s_addr_bytes)[2], ((uint8_t *)hdr->l2_hdr.s_addr_bytes)[3], ((uint8_t *)hdr->l2_hdr.s_addr_bytes)[4], ((uint8_t *)hdr->l2_hdr.s_addr_bytes)[5],
				((uint8_t *)hdr->l2_hdr.d_addr_bytes)[0], ((uint8_t *)hdr->l2_hdr.d_addr_bytes)[1], ((uint8_t *)hdr->l2_hdr.d_addr_bytes)[2], ((uint8_t *)hdr->l2_hdr.d_addr_bytes)[3], ((uint8_t *)hdr->l2_hdr.d_addr_bytes)[4], ((uint8_t *)hdr->l2_hdr.d_addr_bytes)[5],
				hdr->l4_hdr.src_port,
				hdr->l4_hdr.dst_port
			);
#endif
 ...
 * @param num_pkts Number of packets
 */
__global__ void send_packets_kernel(struct doca_gpu_eth_txq *txq, struct doca_gpu_buf_arr *buf_arr, uint32_t gpu_pkt0_idx, const size_t num_pkts, uint32_t max_pkts, uint32_t *gpu_pkts_len, const bool set_completion)
{
	...
	while (pkt_idx < num_pkts) {
		...
		if (ret != DOCA_SUCCESS) {
			printf("Error %d doca_gpu_dev_buf_get_buf thread %d pkt_idx %d gpu_pkt0_idx %d\n",
					ret, threadIdx.x, pkt_idx, gpu_pkt0_idx);
			break;
		}
		...
		printf("num_pkts %ld pkt %d addr %lx %x%x%x%x%x%x %x%x%x%x%x%x - len %d\n",
			num_pkts, pkt_idx + gpu_pkt0_idx, buf_addr,
			((uint8_t*)buf_addr)[0], ((uint8_t*)buf_addr)[1], ((uint8_t*)buf_addr)[2],
			((uint8_t*)buf_addr)[3], ((uint8_t*)buf_addr)[4], ((uint8_t*)buf_addr)[5],
			((uint8_t*)buf_addr)[6], ((uint8_t*)buf_addr)[7], ((uint8_t*)buf_addr)[8],
			((uint8_t*)buf_addr)[9], ((uint8_t*)buf_addr)[10], ((uint8_t*)buf_addr)[11],
			gpu_pkts_len[pkt_idx]
		);
#endif
		if (set_completion && pkt_idx == (num_pkts-1))
			ret = doca_gpu_dev_eth_txq_send_enqueue_weak(txq, buf, gpu_pkts_len[pkt_idx], ((curr_position + pkt_idx) & mask_max_position), DOCA_GPU_SEND_FLAG_NOTIFY);
		else
			ret = doca_gpu_dev_eth_txq_send_enqueue_weak(txq, buf, gpu_pkts_len[pkt_idx], ((curr_position + pkt_idx) & mask_max_position), 0);
		if (ret != DOCA_SUCCESS) {
	...
	if (threadIdx.x == 0) {
		...
		// 	(uint32_t)num_pkts, gpu_pkt0_idx, max_pkts, ((curr_position + num_pkts) & mask_max_position), gpu_pkts_len[num_pkts-1]);
...
doca_error_t doca_receiver_packet_kernel(cudaStream_t stream, int rxqn, uintptr_t *eth_rxq_gpu, uintptr_t *sem_gpu, uint32_t* batch_list, uint32_t *gpu_exit_condition)
{
	...
	if (cudaSuccess != result) {
		HOLOSCAN_LOG_ERROR("[{}:{}] cuda failed with {} \n", __FILE__, __LINE__, cudaGetErrorString(result));
		return DOCA_ERROR_BAD_STATE;
	}

	/* For simplicity launch 1 CUDA block with 32 CUDA threads */
	receive_packets_kernel<<<rxqn, CUDA_BLOCK_THREADS, 0, stream>>>(rxqn, eth_rxq_gpu, sem_gpu, batch_list, gpu_exit_condition);
	result = cudaGetLastError();
	if (cudaSuccess != result) {
		HOLOSCAN_LOG_ERROR("[{}:{}] cuda failed with {} \n", __FILE__, __LINE__, cudaGetErrorString(result));
		return DOCA_ERROR_BAD_STATE;
...
doca_error_t doca_sender_packet_kernel(cudaStream_t stream, struct doca_gpu_eth_txq *txq, struct doca_gpu_buf_arr *buf_arr, uint32_t gpu_pkt0_idx, const size_t num_pkts, uint32_t max_pkts, uint32_t *gpu_pkts_len, bool set_completion)
{
	...
	// fprintf(stderr, "New send kernel gpu_pkt0_idx %d num_pkts %zd max_pkt %d\n", gpu_pkt0_idx, num_pkts, max_pkts);

	/* For simplicity launch 1 CUDA block with 32 CUDA threads */
	send_packets_kernel<<<1, CUDA_BLOCK_THREADS, 0, stream>>>(txq, buf_arr, gpu_pkt0_idx, num_pkts, max_pkts, gpu_pkts_len, set_completion);
	result = cudaGetLastError();
	if (cudaSuccess != result) {
		HOLOSCAN_LOG_ERROR("[{}:{}] cuda failed with {} \n", __FILE__, __LINE__, cudaGetErrorString(result));
		return DOCA_ERROR_BAD_STATE;
