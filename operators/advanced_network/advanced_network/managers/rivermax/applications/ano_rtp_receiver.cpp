...
ReturnStatus ANORTPReceiverApp::initialize_connection_parameters() {
  // RTP receiver currently supports a single NIC: validate all streams share the same local_ip
  const std::string& expected_local_ip =
      m_rtp_receiver_settings->thread_settings[0].stream_network_settings[0].local_ip;
  for (const auto& thread : m_rtp_receiver_settings->thread_settings) {
    for (const auto& stream : thread.stream_network_settings) {
      if (stream.local_ip != expected_local_ip) {
        std::cerr << "ANORTPReceiverApp: all streams must share the same local_ip (single-NIC). "
                  << "Stream " << stream.stream_id
                  << " has local_ip=" << stream.local_ip
                  << " but expected " << expected_local_ip << std::endl;
        return ReturnStatus::failure;
      }
    }
  }
  ...
  if (inet_pton(AF_INET, expected_local_ip.c_str(), &device_address) != 1) {
    std::cerr << "Failed to parse address of device " << expected_local_ip << std::endl;
    return ReturnStatus::failure;
  }
  rmx_status status = rmx_retrieve_device_iface_ipv4(&device_iface, &device_address);
  if (status != RMX_OK) {
    std::cerr << "Failed to get device: " << expected_local_ip
              << " with status: " << status << std::endl;
    return ReturnStatus::failure;
  }
  m_devices_ips.push_back(expected_local_ip);
