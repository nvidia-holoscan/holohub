...
ReturnStatus ANOMediaSenderApp::initialize_sender_threads() {
  // Media sender currently supports a single NIC: validate all streams share the same local_ip
  const std::string& expected_local_ip =
      m_media_sender_settings->thread_settings[0].stream_network_settings[0].local_ip;
  for (const auto& thread : m_media_sender_settings->thread_settings) {
    for (const auto& stream : thread.stream_network_settings) {
      if (stream.local_ip != expected_local_ip) {
        std::cerr << "ANOMediaSenderApp: all streams must share the same local_ip (single-NIC). "
                  << "Stream " << std::to_string(stream.stream_id) << " has local_ip="
                  << stream.local_ip << " but expected " << expected_local_ip << std::endl;
        return ReturnStatus::failure;
      }
    }
  }
  m_app_settings->local_ip = expected_local_ip;
  m_app_settings->source_port = 0;
  // These are not really used, but required by the API
