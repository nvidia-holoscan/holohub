...
void ANOMediaSenderApp::initialize_sender_threads()
{
  ...
  for (int thread_idx = 0; thread_idx < m_media_sender_settings->num_of_threads; ++thread_idx) {
    // This is not used, just set to the first stream's destination IP and port
    m_app_settings->destination_ip = m_media_sender_settings->thread_settings[thread_idx].stream_network_settings[0].destination_ip;
    m_app_settings->destination_port = m_media_sender_settings->thread_settings[thread_idx].stream_network_settings[0].destination_port;
