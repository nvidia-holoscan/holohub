  ...
  for (const auto& thread : settings->thread_settings) {
    ...
    for (const auto& stream : thread.stream_network_settings) {
      ...
      for (size_t i = 0; i < stream.source_ips.size(); ++i) {
        ReturnStatus rc = ValidatorUtils::validate_ip4_address(stream.source_ips[i]);
        if (rc != ReturnStatus::success) {
          return rc;
        }
        rc = ValidatorUtils::validate_ip4_address(stream.destination_ips[i]);
        if (rc != ReturnStatus::success) {
          return rc;
        }
        rc = ValidatorUtils::validate_ip4_address(stream.local_ips[i]);
        if (rc != ReturnStatus::success) {
          return rc;
        }
        rc = ValidatorUtils::validate_ip4_port(stream.destination_ports[i]);
        if (rc != ReturnStatus::success) {
          return rc;
        }
      }
...
ReturnStatus ANOIPOReceiverApp::initialize_connection_parameters() {
  ...
  for (const auto& thread : m_ipo_receiver_settings->thread_settings) {
    for (const auto& stream : thread.stream_network_settings) {
      num_of_paths = stream.local_ips.size();
