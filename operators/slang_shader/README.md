...
- **IGX Thor iGPU**: Not supported. The Slang compiler bundled with this operator emits PTX that
  the IGX Thor iGPU driver cannot load, so `SlangShaderOp` fails at setup with
  `the provided PTX was compiled with an unsupported toolchain. (222)`. See
...
5. **`the provided PTX was compiled with an unsupported toolchain. (222)`**: The PTX generated by
   the bundled Slang compiler is not accepted by the CUDA driver on the current GPU. This is a known
   limitation on the IGX Thor iGPU, which is not supported by this operator. Run on a supported
   platform (x86_64 or aarch64 with a supported discrete GPU) instead.
