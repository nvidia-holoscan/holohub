...
VideoInfo::VideoInfo(const Caps& caps) {
  if (!gst_video_info_from_caps(&video_info_, caps.get()))
    throw std::runtime_error("Failed to create VideoInfo from Caps - caps may not contain valid video information");
}
...
gsize VideoInfo::get_plane_comp_height(int plane_index) const {
  if (plane_index < 0 || static_cast<guint>(plane_index) >= video_info_.finfo->n_planes) {
    return 0;
  }
...
gsize VideoInfo::get_plane_size(int plane_index) const {
  ...
  return get_plane_stride(plane_index) * get_plane_comp_height(plane_index);
}
