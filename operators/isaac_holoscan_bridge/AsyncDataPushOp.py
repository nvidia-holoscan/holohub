...
class AsyncDataPushOp(Operator):
    ...
    def __init__(
        self, fragment, outputs: list[str], max_queue_size: int = 0, *args, **kwargs
    ):
    ...
    def compute(self, op_input, op_output, context):
        ...
        for output_name, data in data_dict.items():
            # Check if data is a array and emit as tensor
            if (
                ...
                op_output.emit({"": Tensor.as_tensor(data)}, output_name)
            else:
                op_output.emit({"": data}, output_name)
