...
def git_pieces_from_vcs(tag_prefix, root, verbose, runner=run_command):
    ...
    describe_out, rc = runner(
        GITS,
        ["describe", "--tags", "--dirty", "--always", "--long", "--match", f"{tag_prefix}*"],
        cwd=root,
    )
...
def render_pep440_branch(pieces):
    ...
    if pieces["closest-tag"]:
        ...
        if pieces["branch"] != "master":
            rendered += ".dev0"
        rendered += f"+untagged.{pieces['distance']:d}.g{pieces['short']}"
        if pieces["dirty"]:
            rendered += ".dirty"
    return rendered
...
def render_pep440_pre(pieces):
    ...
    if pieces["closest-tag"]:
        rendered = pieces["closest-tag"]
        if pieces["distance"]:
            rendered += f".post0.dev{pieces['distance']:d}"
    else:
        # exception #1
        rendered = f"0.post0.dev{pieces['distance']:d}"
    return rendered
