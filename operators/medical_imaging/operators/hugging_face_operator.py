...
class HuggingFacePipelineOperator(Operator):
    ...
    def __init__(self, fragment: Fragment, *args, pipeline_type:str, pipeline_name:str, pipeline_uri: os.PathLike, revision: str | None = None, pipeline_init_kwargs: Dict = {}, **kwargs):
        self.pipeline_type = pipeline_type
        if self.pipeline_type not in {item.value for item in HF_Pipeline_Type}:
            raise AttributeError(f"Invalid pipeline type, expecting {self.HUGGING_FACE_PIPELINE_TYPES}, but got {self.pipeline_type}")
