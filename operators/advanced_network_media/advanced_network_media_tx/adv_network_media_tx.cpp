 ...
 public:
  ...
  void process_input(InputContext& op_input) {
    ...
    if (pending_tx_frame_) {
      ...
      dropped_frames++;
      ANM_LOG_WARN(
          "Dropping unsent frame (total dropped: {}). TX queue {}, port {} may be "
          "congested or too slow.",
          dropped_frames, parent_.queue_id_.get(), port_id_);
      pending_tx_frame_ = nullptr;  // Clear the old frame
    }
