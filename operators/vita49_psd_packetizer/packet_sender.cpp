...
PacketSender::PacketSender(const char *dest_host,
                           unsigned short dest_port,
                           uint32_t _manufacturer_oui,
                           uint32_t _device_code) {
    dest_host_resolved = lookup_host(dest_host);
...
uint64_t PacketSender::get_packet_count() {
    return packet_count;
}
...
int PacketSender::send_context_packet(std::shared_ptr<holoscan::MetadataDictionary> meta) {
    ...
    if (meta->has_key("context_field_change"))
        context_packet.change_indicator(meta->get<bool>("context_field_change"));
