...
def main(graphs, constraint = False):
    ...
    for i, data in enumerate(graphs[0]):
        ...
        plt.figure(0)
        plt.scatter(observedvals, predictedvals, color=colors[i], marker=markers[i], label=labels[i], s=30)
        plt.figure(1)
        plt.scatter(simulvals, predictedvals, color=colors[i], marker=markers[i], label=labels[i], s=30)
...
def overheadmain():
    ...
    plt.plot(range(1,12), observedoverheads, color='red', marker='o', linestyle='--', label='Observed WCRT')
...
def scalabilitypess():
    ...
    with open('predictedresponsetimes.txt', 'r') as predicted, open('simulatedresponsetimes.txt', 'r') as simul:
