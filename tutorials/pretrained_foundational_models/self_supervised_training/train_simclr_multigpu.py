...
class SurgicalVisionDataset_json(data.Dataset):
    def __init__(self, csv_path, transform=None, contrastive_training=True,
                 train_set=True, loader=pil_loader):
...
class SimCLR(pl.LightningModule):
    def __init__(self, hidden_dim, lr, temperature, weight_decay, max_epochs=500, backbone='resnet18'):
        super().__init__()
...
def main():
    ...
    parser.add_argument("--gpus", type=int, choices=[1, 2, 3, 4], 
        help="Number of GPUs to use in training", default=4)
    parser.add_argument("--epochs", type= int, help="Number of epochs you'd like to train", default=300)
    parser.add_argument("--backbone", type=str, help="Model architecture you wish to train", default='resnet18')
    ...
    #launch training
    train_simclr(batch_size=args['batch_size'], hidden_dim=128, lr=5e-4*args['gpus'], temperature=0.07, weight_decay=1e-4,
        max_epochs=args['epochs'], num_GPUS=args['gpus'], backbone=args['backbone'])
...
if __name__ == '__main__':
    ...
    unlabeled_data = SurgicalVisionDataset_json(csv_path=DATASET_PATH+"activ.csv",
