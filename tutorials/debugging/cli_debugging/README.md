...
### Frequently Asked Questions and Troubleshooting (C++ gdb)
...
We can use `info sources` to inspect the Holoscan SDK symbols are available. __Note__: Source paths are loaded from
...
1. Press `Ctrl+C` to return to your interactive shell
...
1. Attach to the running process with `gdb -p`:
...
### Frequently Asked Questions and Troubleshooting (Legacy Holoscan SDK)
