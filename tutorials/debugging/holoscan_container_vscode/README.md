...
> 💡 Note: The Dev Container is configured with a default user account `holoscan` using user ID `1000` and group ID `1000` in the [devcontainer.json](./.devcontainer/devcontainer.json) file.
>
> - To change the user name, find and replace `USERNAME` and `remoteUser`.
> - To change the user ID, find and replace `USER_UID` and `userUid`.
> - To change the group ID, find and replace `USER_GID` and `userGid`.
...
Open the [hello_world.cpp](https://github.com/nvidia-holoscan/holoscan-sdk/blob/main/examples/hello_world/cpp/hello_world.cpp#L55) file. (In VSCode, click on **Explorer** from the sidebar and expand the **holoscan-sdk** folder. Then, find and open `hello_world.cpp` under the `examples/hello_world/cpp` directory.)
