...
class HDF5ProcessingApp(Application):
  def compose(self):
    ...
    reader = ReaderOperator(self, CountCondition(self, 100), file_path=str(config['hdf5_reader']['file_path']), name = 'hdf5_reader')
    threshold_sum = ThresholdSumOperator(self, threshold=config['threshold_sum']['threshold'], name = 'threshold_sum')
