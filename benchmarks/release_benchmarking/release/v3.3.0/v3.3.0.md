...
- We ran the C++ version of the applications. 
...
- We included a long-running benchmark for Endoscopy Tool Tracking.
    ...
    - 1-2 instances on x86_64 with ADA6000 GPU
...
Overall, a major improvement over previous versions is that we no longer see large spikes in maximum latencies. This is perhaps due to improvements in our driver software. We observe improvement in performance compared to v3.0 in the experiments with "As Fast As Possible" mode for both applications. For this report, we add an additional python and event-based scheduler benchmark. We observe that the performance of the python version is comparable to the C++ version in most cases. Though the average latency of the python version is slightly better, the 99.9th percentile and maximum latencies are slightly worse (Ex: 63.40ms vs 38.86ms on x86_64 (A6000)). 
...
#### Real-Time Video Input - C++ Version
...
#### "As Fast As Possible" Video Input - C++ Version
...
#### "As Fast As Possible" Video Input and Headless Output - C++ Version
