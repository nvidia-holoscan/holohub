...
def get_max_latencies(paths_latencies, skip_begin_messages = 10, discard_last_messages = 10):
    max_latencies = {}
    for path in paths_latencies:
        max_latencies[path] = round(max(paths_latencies[path][skip_begin_messages:-discard_last_messages]), 2)
...
def main():
    ...
    parser.add_argument("--save-csv", action="store_true", help="save the respective values (max, avg, median, etc.) of the first path for every group in a CSV file in comma-separated format. (avg: avg_values.csv, max: max_values.csv, gpu utilization: avg_gpu_utilization_values.csv)", required=False)

    parser.add_argument("--draw-cdf", nargs="?", type=str, const="cdf_curve.png", help="draw an end-to-end latency CDF curve for the first path of each group of log files. An (optional) filename could also be provided where the graph will be saved.", required=False)
        ...
        for group_name, paths_latencies in grouped_path_latenices.items():
            ...
            if args.save_csv:
                with open("median_values.csv", "a") as f:
                    f.write(str(round(np.median(paths_latencies[list(paths_latencies.keys())[0]]), 2)) + ",")
    ...
    if args.tail:
        if args.save_csv:
            with open("tail_values.csv", "w") as f:
                f.truncate(0)
        print_metric_title("Latency CDF Curve Tail (95-100 percentile)")
        for group_name, paths_latencies in grouped_path_latenices.items():
            print_group_name_with_log_files(group_name, grouped_log_files[group_name])
            for path, latency in paths_latencies.items():
    ...
    if args.flatness:
        if args.save_csv:
            with open("flatness_values.csv", "w") as f:
                f.truncate(0)
        print_metric_title("Latency CDF Curve Flatness (10-90 percentile)")
        for group_name, paths_latencies in grouped_path_latenices.items():
            print_group_name_with_log_files(group_name, grouped_log_files[group_name])
            for path, latency in paths_latencies.items():
