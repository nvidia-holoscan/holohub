...
class HoloHubCLI:
    ...
    def build_project_locally(
        ...
        project_deps = parse_module_dependencies(
            metadata_path, holohub_root=HoloHubCLI.HOLOHUB_ROOT
        )
    ...
    def handle_list(self, args: argparse.Namespace) -> None:
        ...
        for project_type in LIST_TYPES:
            ...
            for project in sorted(grouped_metadata[project_type], key=lambda x: x["project_name"]):
                ...
                if project_type == "module":
                    operators = (
                        meta.get("operator_names")
                        or meta.get("operators")
                        or meta.get("subprojects", {}).get("operators", [])
                    )
    ...
    def handle_create(self, args: argparse.Namespace) -> None:
        ...
        # the HoloHub tree), so we prompt interactively when --directory is not supplied.
        if args.directory is None:
            if is_module_template:
                raw = input("Output directory for the new module: ").strip()
                if not raw:
                    holohub_cli_util.fatal("Output directory is required for module templates.")
                args.directory = Path(raw).expanduser().resolve()
            else:
                args.directory = self.HOLOHUB_ROOT / "applications"
        ...
        if is_module_template:
            msg_next = (
                ...
                f"- Build and test with HoloHub CLI\n"
            )
