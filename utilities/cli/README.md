...
For implementation details, workflow tips, and extension guide see the [CLI Developer Guide](CLI_DEV_GUIDE.md).
...
Commands flow through six internal components: **Input Resolution** (parsing, fuzzy matching) → **Project Resolution** (metadata scanning) → **Mode Resolution & Config Merge** (mode + CLI flag merge) → **Container Build & Run** (Docker orchestration) → **Application Build & Run** (CMake, path expansion) → **Execution & Error Handling** (dry-run, validation).
