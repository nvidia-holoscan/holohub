...
| `HOLOSCAN_CLI_INSTALL_ARGS` | Whitespace-separated pip options and optional custom requirements. HoloHub uses pip's default package index and adds one exact requirement for its committed pin (or `holoscan-cli>4.2.0` in floating mode). Shell quotes are not parsed. |
