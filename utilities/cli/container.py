...
class HoloHubContainer:
    ...
    def build(
        ...
        if extra_scripts:
            cmd.extend(["-t", f"{primary_tag}-base"])
        ...
        if extra_scripts:
            for script in extra_scripts:
                ...
                cmd = [
                    ...
                    "BUILDKIT_INLINE_CACHE=1",
                    "--build-arg",
                    f"BASE_IMAGE={primary_tag}", # reuse the default tag to sequentially add the scripts on top of each other.
                    "--network=host",
                    "--build-arg",
                    f"SCRIPT={relative_script_path}",
                    "-t",
                    f"{primary_tag}-{script}",
                    "-t",
                    f"{primary_tag}", # override the default tag so we can add the next scripts on top of this.
                    "-f",
