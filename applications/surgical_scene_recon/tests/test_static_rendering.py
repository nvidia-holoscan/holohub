...
class StaticRenderingTestApp(Application):
    ...
    def compose(self):
        ...
        debug = DebugPrintOp(self, name="debug", print_every=5, print_first=2)
...
def main():
    ...
    parser.add_argument(
        "--num_frames", type=int, default=5, help="Number of frames to render (default: 5)"
    )
    ...
    try:
        app = StaticRenderingTestApp(
            data_dir=args.data_dir, checkpoint_path=args.checkpoint, num_frames=args.num_frames
        )
        app.run()
