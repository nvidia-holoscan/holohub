...
class EndoRunner:
    ...
    def __init__(
        ...
        if cfg.use_deformation:
            ...
            print(
                f"[Phase 6]   MLP parameters: {sum(p.numel() for p in self.deform_net.get_mlp_parameters())}"
            )
            print(
                f"[Phase 6]   Grid parameters: {sum(p.numel() for p in self.deform_net.get_grid_parameters())}"
            )
        ...
        print(
            f"[Phase 1] Configuration: two_stage={cfg.two_stage_training}, "
            f"depth_mode={cfg.depth_mode}, use_deformation={cfg.use_deformation}"
        )
        print(
            f"[Priority 2] Accurate mask: {cfg.accurate_mask}, TV on occluded only: {cfg.tv_on_occluded_only}"
        )
        print(
            f"[Phase 1] Masking: use_mask={cfg.use_mask}, enable_tool_masking={cfg.enable_tool_masking}, "
    ...
    def _train_stage(self, stage: str, num_iterations: int):
        ...
        for step in pbar:
            ...
            if (
                ...
                print(f"\n[MONITOR] Step {step}:")
                print(
                    f"  Image rendered range: [{image_rendered.min():.3f}, {image_rendered.max():.3f}]"
                )
                print("  Gaussian parameters:")
                print(
                    f"    means range: [{self.splats['means'].min():.3f}, {self.splats['means'].max():.3f}]"
                )
                print(
                    f"    scales (raw) range: [{self.splats['scales'].min():.3f}, {self.splats['scales'].max():.3f}]"
                )
                print(
                    f"    opacities (raw) range: [{self.splats['opacities'].min():.3f}, {self.splats['opacities'].max():.3f}]"
                )
                print(
                    f"    opacities (activated) range: [{torch.sigmoid(self.splats['opacities']).min():.6f}, {torch.sigmoid(self.splats['opacities']).max():.6f}]"
                )
                ...
                if hasattr(self, "_deformation_accum"):
                    ...
                    print(
                        f"    Accumulated magnitude: mean={deform_mag.mean():.6f}, max={deform_mag.max():.6f}"
                    )
                    print(f"    Active deformations: {deform_count}/{len(deform_mag)}")
                    print(
                        f"    Deformable Gaussians: {self._deformation_table.sum()}/{len(self._deformation_table)}"
                    )
            ...
            if cfg.tv_on_occluded_only and self.invisible_mask is not None:
                ...
                if image_rendered_p.shape[0] > 1:
                    inpaint_mask_batch = inpaint_mask_batch.repeat(
                        image_rendered_p.shape[0], 1, 1, 1
                    )
                ...
                if step % 100 == 0 and self.world_rank == 0:
                    print(
                        f"[Priority 2] Targeted TV loss: image={tv_image.item():.6f}, depth={tv_depth.item():.6f}"
                    )
            else:
            ...
            if (
                cfg.enable_tool_masking
                and stage == "fine"
                and step % cfg.tool_mask_update_interval == 0
                and step > 0
            ):
            ...
            if self.world_rank == 0 and cfg.tb_every > 0 and step % cfg.tb_every == 0:
                ...
                if hasattr(self, "_deformation_accum") and stage == "fine":
                    ...
                    self.writer.add_scalar(
                        f"{stage}/deform_magnitude_mean", deform_mag.mean().item(), step
                    )
                    self.writer.add_scalar(
                        f"{stage}/deform_magnitude_max", deform_mag.max().item(), step
                    )
    ...
    def _save_checkpoint(self, step: int, stage: str, is_best: bool = False):
        ...
        if is_best:
            print(
                f"[BEST] New best PSNR: {self.best_psnr:.3f} at step {step} - Checkpoint saved: {checkpoint_path}"
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_type", type=str, default="endonerf", choices=["endonerf", "scared"]
    )
    ...
    parser.add_argument(
        "--no_mask", action="store_true", help="Disable GT masking (tools visible in training)"
    )
    parser.add_argument(
        "--enable_tool_masking",
        action="store_true",
        help="Enable tool masking for tissue reconstruction",
    )
    parser.add_argument(
        "--tool_mask_threshold",
        type=int,
        default=1,
        help="Min frames in tool region to mark as non-deformable",
    )
    parser.add_argument(
        "--tool_mask_update_interval",
        type=int,
        default=500,
        help="Update deformation table every N steps",
    )
    parser.add_argument(
        "--depth_supervise_tools",
        action="store_true",
