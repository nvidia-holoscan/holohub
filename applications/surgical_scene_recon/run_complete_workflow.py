...
def run_training(ingestion_dir, output_dir, training_iterations, coarse_iterations, use_masks):
    ...
    cmd = [
        sys.executable,
        str(training_script),
        "--data_dir", str(ingestion_dir),
        "--result_dir", str(training_output),
        "--max_steps", str(training_iterations),
        "--coarse_iterations", str(coarse_iterations),
        "--two_stage",
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="output/complete_workflow", help="Output directory")
    ...
    parser.add_argument("--train_only", action="store_true", help="Only run training, skip rendering")
