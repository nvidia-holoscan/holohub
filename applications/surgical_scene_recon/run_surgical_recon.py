...
def run_inference_mode(args):
    ...
    rendering_script = Path("dynamic_rendering_viz.py")
...
def main():
    ...
    parser.add_argument(
        "--loop", action="store_true", default=False, help="Loop rendering continuously"
    )

    # Inference-only mode arguments
    parser.add_argument(
        "--checkpoint", type=str, help="Path to checkpoint file (required for inference mode)"
    )
