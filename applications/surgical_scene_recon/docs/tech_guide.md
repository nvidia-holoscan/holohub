...
**Entry point:** `surgical_scene_recon.py` (Holoscan Application)
...
3. **SAM3 checkpoint missing or 0 bytes** — Place the real checkpoint in the same data tree as your frames (e.g. `data/surgical_scene_recon/medsam3/checkpoint.pt`) or bind-mount and pass `--sam3-checkpoint`.
...
├── surgical_scene_recon.py          — Phase 1: Holoscan streaming app
│
