...
class TAPNext(nn.Module):
    ...
    def prediction_heads(self, x):
        ...
        argmax_x, argmax_y = position_x.argmax(dim=-1, keepdim=True), position_y.argmax(
            dim=-1, keepdim=True
        )
        index = torch.arange(position_x.shape[-1], device=x.device).repeat(*argmax_x.shape[:-1], 1)
    ...
    def forward(
        ...
        video_reshaped = video.permute(0, 1, 4, 2, 3).reshape(
            b * t, 3, video.shape[2], video.shape[3]
        )
