...
from torch.utils._pytree import _register_namedtuple
...
class TRecViTBlock(nn.Module):
  ...
  def __init__(self, depth, width, num_heads, lru_width, dtype, device, n):
    super().__init__()
    ...
    b = 1
    self.to_bn_tc = Rearrange('b t n c -> (b n) t c')
...
class TAPNext(nn.Module):
  ...
  def __init__(
    ...
    n = image_size[0] // patch_size[0] * image_size[1] // patch_size[1] + n_query_points
  ...
  def forward(self, video: torch.Tensor, query_points: Optional[torch.Tensor] = None, state: Optional[Dict] = None):
    # video.shape
    b, t, _, _, _ = video.shape
    # [b, t, h, w, 3] -> [b, t, 3, h, w]
    # Reshape video from [b, t, h, w, c] to [b * t, c, h, w] for Conv2d
    video_reshaped = video.permute(0, 1, 4, 2, 3).reshape(b * t, 3, video.shape[2], video.shape[3])
    video_tokens = self.lin_proj(video_reshaped)
    _, _, h, w = video_tokens.shape
    # Reshape back to [b, t, h * w, c]
    video_tokens = video_tokens.permute(0, 2, 3, 1).reshape(b, t, h * w, -1)
    ...
    if state is not None:
        ...
        step = torch.tensor((0,), device=video.device, dtype=torch.int32)
