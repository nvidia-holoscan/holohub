...
class TapNextONNXWrapper(torch.nn.Module):
    ...
    def forward(self, 
        ...
        if step is not None:
            ...
            tracking_state = {
                ...
                "hidden_state": [
                    ...
                    for state in zip(torch.unbind(rg_lru_state, 0),
                             torch.unbind(conv1d_state, 0))
                ],
            }
            query_points = None
        else:
        ...
        curr_tracks, curr_track_logits, curr_visible_logits, tracking_state = self.model(video, query_points=query_points, state=tracking_state)
        ...
        rg_lru_state = torch.stack([state["rg_lru_state"] for state in tracking_state["hidden_state"]], dim=0)
        conv1d_state = torch.stack([state["conv1d_state"] for state in tracking_state["hidden_state"]], dim=0)
...
def export_to_onnx(model, video_tensor, output_path, query_points, step=None, rg_lru_state=None, conv1d_state=None):
    ...
    if step is None:
        ...
        torch.onnx.export(
            ...
            output_names=["tracks", "track_logits", "visible_logits", "step", "rg_lru_state", "conv1d_state"],
            # dynamo=True
        )
    else:
        # Export to ONNX
        torch.onnx.export(
            model,
            args=(),
            kwargs=dict(video=video_tensor, query_points=query_points, step=step, rg_lru_state=rg_lru_state, conv1d_state=conv1d_state),
            f=os.path.join(output_path, name),
            export_params=True,
            opset_version=20,
            do_constant_folding=True,
...
def main():
    parser = argparse.ArgumentParser(
        description="Export TapNext model to ONNX and validate"
    )
    parser.add_argument(
        "--checkpoint",
    ...
    # )
    parser.add_argument(
        "--output_path", default="output/", help="Output ONNX file path"
    )
    parser.add_argument(
        "--grid_size", type=int, default=15, help="Grid size for query points"
    )
    parser.add_argument(
        "--query_frame", type=int, default=0, help="Query frame index"
    )
    parser.add_argument(
        "--device", default="cpu", help="Device to use (cpu or cuda)"
    )
    ...
    tapnext_model = TAPNext(
        image_size=(256, 256), 
        n_query_points=args.grid_size * args.grid_size,
        device=device
    )
    tapnext_model = restore_model_from_jax_checkpoint(
        tapnext_model, args.checkpoint
    )
    ...
    query_points = create_query_points(args.grid_size, query_frame=args.query_frame, height=256, width=256)
    ...
    export_to_onnx(model, video_tensor[:, 1:2], args.output_path, query_points, step=step, rg_lru_state=rg_lru_state, conv1d_state=conv1d_state)
