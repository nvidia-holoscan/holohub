...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        """Calls the RMSNorm."""
        var = torch.mean(
            torch.square(x), dim=-1, keepdim=True
        )  # This was dim=-1 in the original code
...
class CausalConv1D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cache: torch.Tensor | None = None):
        ...
        if one_step:
            y = (x * self.w.unsqueeze(0)).sum(1, keepdim=True) + self.b[None, None, :]
        else:
