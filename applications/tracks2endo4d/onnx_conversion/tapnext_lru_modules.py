...
class RMSNorm(nn.Module):
  ...
  def forward(self, x):
    """Calls the RMSNorm."""
    var = torch.mean(torch.square(x), dim=-1, keepdim=True)  # This was dim=-1 in the original code
...
class BlockDiagonalLinear(nn.Module):
  ...
  def forward(self, x):
    ...
    n, t, last = x.shape
    x = x.reshape(n, t, self.num_blocks, last // self.num_blocks)
    ...
    # Flatten the output: merge the last two dimensions (..., h, j) -> (..., h * j)
    y = y.reshape(n, t, self.num_blocks * self.block_width)
    return y
    


def rnn_scan(x: torch.Tensor, a: torch.Tensor, h0: torch.Tensor | None, acc_dtype: torch.dtype = torch.float32, use_linear_scan: bool = True):
...
class RGLRU(nn.Module):
  ...
  def forward(self, x: torch.Tensor, cache: torch.Tensor | None = None, use_linear_scan: bool = True):
    _, l, _ = x.shape
...
class CausalConv1D(nn.Module):
  ...
  def forward(self, x: torch.Tensor, cache: torch.Tensor | None = None):
    ...
    if one_step:
      y = (x * self.w.unsqueeze(0)).sum(1, keepdim=True) + self.b[
          None, None, :
      ]
    else:
      y = F.conv1d(
