...
![Example output for BCI Visualization](docs/brain_activity_example.gif)

## Example 3D visualization

## Overview

This HoloHub application demonstrates real-time source reconstruction and 3D visualization for brain-computer interface (BCI) pipelines built with NVIDIA Holoscan. It streams functional brain data from the Kernel Flow 2 system and was developed and tested on an NVIDIA Jetson Thor paired with a Kernel Flow 2 headset. To lower the barrier to entry, we also provide recorded datasets and a data replayer, enabling developers to build and experiment with visualization and classification pipelines without requiring access to the hardware.
...
![Monte Carlo simulation of photon scattering events](docs/photon_simulation.gif)
...
![Flow's 3000+ channels](docs/flow_channel_map.png)
