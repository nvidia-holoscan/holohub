...
class Signal_GeneratorOp(Operator):
    ...
    fft_input         : flag indicating whether input data should be fourier transformed before emission
    ...
    Output
    ------
    xyz          : numpy vector represnting the location of the receiver for the current pulse
    ...
    def __init__(self, *args, bfile, num_pulses=-1, num_samples=-1, oversample_factor=1, \
            Input_Filename="", Fourier_Transform_Input=-1, File_Loop=0, Target_PRF=-1, Abort_After=-1, dtype=cp.int32, **kwargs):
    ...
    def compute(self, op_input, op_output, context):
        ...
        if (self.time_per_pulse > 0):
            ...
            if (elapsed_time > self.time_per_pulse * (self.total_count+1)):
               print ("Late Sending pulse ", self.total_count, " at ", elapsed_time, "scheduled for ", self.time_per_pulse * self.total_count)
...
class BP_Image_FormationOp (Operator):
    """Form an image of the ground plane of interest via backprojection.
    ...
    Output
    ------
    xyz          : numpy vector represnting the location of the receiver for the most recently consumed pulse
    image        : numpy matrix of complex values representing reflectivity at points of interest

    """

    def __init__(self, *args, num_samples=-1, dr=-1, \
            Image_Size_X=-1, \
            Image_Size_Y=-1, \
            Pixel_Spacing=-1, \
        ...
        xx = cp.linspace(min_x, min_x + self.pixel_spacing*(self.image_size_x-1), self.image_size_x)
    ...
    def pulse_bit (self, pulse, factor):
        xyz = pulse["xyz"]
        r0 = pulse["r1"]
        N = pulse["sample_count"]
        input_samples = pulse["samples"]
        pixel_radius = cp.sqrt((self.coords[0]-xyz[0])**2 + (self.coords[1]-xyz[1])**2 + xyz[2]**2)-r0
...
class Image_OutputOp(Operator):
    ...
    def compute(self, op_input, op_output, context):
        ...
        v1 = xyz[:2]
        v2 = self.first_xyz[:2]
        theta = cp.arccos (cp.dot(v1, v2) / (cp.linalg.norm(v1) * cp.linalg.norm(v2)))*180/PI
        ...
        text = "TX: (" + '%4.2f' % xyz[0] + ", " + '%4.2f' % xyz[1] + ") " + \
             " Pulses=" + str(self.count) # + " angle=" + '%4.2f' % theta
        im1.text ((5,5), text, font=mf)
        #im1.line ((image_size_x/2, image_size_y/2, image_size_x/2+xyz[0], image_size_y/2-xyz[1]), width=3)
        #im1.line ((image_size_x/2, image_size_y/2, image_size_x/2+self.first_xyz[0], image_size_y/2-self.first_xyz[1]), width=3)
        e = Image.Image.getdata(im)
        f=cp.asarray(e, dtype=cp.uint8).reshape (image_size_x, image_size_y)
        f=cp.repeat(f[:,:,cp.newaxis],3,axis=2) 
        filename = self.image_prefix + str(self.count) + ".png"
        #im = im.save (filename)
...
class SAR_ImagingApp(Application):
    ...
    The current implementation is somewhat hard codede for the GOTCHA Volumetric SAR datasets.
    A preprocessor is used to conver the provided data sets from a MATLAB-centric format to
    ...
    def compose(self):
        ...
        algorithm = str(self.from_config("SAR_Image_Formation.Algorithm"))
        ...
        if (1):
            convolver = BP_Image_FormationOp(self, name="conv", num_samples=num_samples, dr=self.dr, \
                    minf=self.minf,**self.kwargs("SAR_Image_Formation"))
        else:
            pass
        printer = Image_OutputOp(self, final=self.num_pulses, name="printer", **self.kwargs("SAR_Output"))
