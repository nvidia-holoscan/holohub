...
class EHRAgent(Agent):
        ...
        def _get_ehr_database(self):
            ...
            chroma_db = Chroma(persist_directory=self.agent_settings.get("db_path"), embedding_function=embedding_model, collection_name="ehr_rag")

            chroma_db = chroma_db.as_retriever(
                search_type="mmr", search_kwargs={"k": self.agent_settings["k"], "lambda_mult" : self.agent_settings["lambda_mult"], "fetch_k" : self.agent_settings["fetch_k"]}
                )
            return chroma_db
        ...
        def process_request(self, text, chat_history):
            ...
            self._logger.debug(f"Documents retreived:")
