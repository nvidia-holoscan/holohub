...
void print_usage() {
  std::cout << "Usage: streaming_client_demo_04_80 [options]\n"
            ...
            << "streaming_client_demo.yaml)\n"
            << "  -d, --data <directory>     Data directory (default: "
            << "environment variable HOLOSCAN_INPUT_PATH or current directory)\n"
            << std::endl;
}
...
template<>
std::string get_config_value<std::string>(
...
int main(int argc, char** argv) {
  ...
  std::string server_ip = get_config_value(
      app.get(),
      "streaming_client.server_ip",
