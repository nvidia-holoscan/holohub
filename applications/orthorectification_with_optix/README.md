...
![Orthorectification pipeline workflow](docs/odm_ortho_pipeline.png)
...
```bash
DOCKER_BUILDKIT=1 docker build -t holohub-ortho-optix:latest .
```

You can now run the docker container by:

1. Allow Docker to use the display:

```bash
xhost +local:docker
```

1. Locate the NVIDIA Vulkan ICD JSON:
...
```bash
docker run -it --rm --net host --runtime=nvidia -v ~/Data:/root/Data  -v .:/work/ -v /tmp/.X11-unix:/tmp/.X11-unix  -v $nvidia_icd_json:$nvidia_icd_json:ro  -e NVIDIA_DRIVER_CAPABILITIES=graphics,video,compute,utility,display -e DISPLAY=$DISPLAY  holohub-ortho-optix
```
...
```bash
gdal_translate -tr 0.25 0.25 -r cubic ~/Data/lafayette_square/odm_dem/dsm.tif ~/Data/lafayette_square/odm_dem/dsm_small.tif
```

1. Fill nodata:

```bash
gdal_fillnodata.py -md 0 ~/Data/lafayette_square/odm_dem/dsm_small.tif ~/Data/lafayette_square/odm_dem/dsm_small_filled.tif
```
...
```bash
python ./python/ortho_with_pyoptix.py
```
...
![Orthorectification sample application](docs/holohub_ortho_app.gif)
