...
class NVIDIAVideoCodecApp(Application):
    ...
    def compose(self):
        ...
        use_host_memory = bool(self.from_config("encoder.copy_to_host"))
        encoder = NvVideoEncoderOp(
            ...
            allocator=BlockMemoryPool(
                self,
                name="pool",
                storage_type=MemoryStorageType.HOST if use_host_memory else MemoryStorageType.DEVICE,
                block_size=source_block_size,
                num_blocks=source_num_blocks,
            ),
            **self.kwargs("encoder"),
        ...
        self.add_flow(format_converter, encoder, {("tensor", "in")})
        self.add_flow(encoder, decoder, {("out", "in")})
        self.add_flow(decoder, visualizer, {("out", "receivers")})
