...
void UcxxEndoscopyServerApp::compose() {
  ...
  // Create allocators
  auto replayer_allocator = make_resource<RMMAllocator>("video_replayer_allocator",
  ...
  auto lstm_inferer = make_operator<ops::LSTMTensorRTInferenceOp>(
      ...
      Arg("pool") = make_resource<BlockMemoryPool>("pool", 1, lstm_inferer_block_size, lstm_inferer_num_blocks),
      Arg("cuda_stream_pool") = cuda_stream_pool);
  ...
  auto tool_tracking_postprocessor = make_operator<ops::ToolTrackingPostprocessorOp>(
      ...
      Arg("device_allocator") = make_resource<BlockMemoryPool>(
          "device_allocator", 1, tool_tracking_postprocessor_block_size, tool_tracking_postprocessor_num_blocks));
  ...
  auto visualizer_allocator = make_resource<BlockMemoryPool>("allocator", 1, render_buffer_size, source_num_blocks);
  ...
  auto render_buffer_converter = make_operator<ops::FormatConverterOp>(
      ...
      Arg("pool") = make_resource<BlockMemoryPool>("pool", 1, render_buffer_size, source_num_blocks),
      Arg("cuda_stream_pool") = cuda_stream_pool);
  ...
  HOLOSCAN_LOG_INFO("Server pipeline: Replayer → Format → LSTM → Postprocess → Holoviz → Convert → Broadcast");
}
