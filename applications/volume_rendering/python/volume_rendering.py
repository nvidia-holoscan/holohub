...
if __name__ == "__main__":
    render_config_file_default = pathlib.Path("../../../data/volume_rendering/config.json").resolve()
    density_volume_file_default = pathlib.Path("../../../data/volume_rendering/highResCT.mhd").resolve()
    mask_volume_file_default = pathlib.Path(
        "../../../data/volume_rendering/smoothmasks.seg.mhd"
    ).resolve()

    parser = argparse.ArgumentParser(description="Volume Rendering Application")
    parser.add_argument(
        "-c",
        "--config",
        action="store",
    ...
    app = VolumeRenderingApp(
        render_config_file=str(args.config), density_volume_file=str(args.density), mask_volume_file=str(args.mask)
    )
    app.config()
