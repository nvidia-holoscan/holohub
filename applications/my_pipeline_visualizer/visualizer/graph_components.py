...
def create_graph(
    ...
    # Create the figure - detect if data is 2D (Ising model) or 1D (time series)
    if data is not None:
        # Debug: print data shape
        print(f"[DEBUG] Data shape for {name}: {data.shape}, ndim: {data.ndim}")
        
        # Check if data is 2D (spatial) or 1D (temporal)
        if data.ndim >= 2 and data.shape[0] > 1 and data.shape[1] > 1:
            # 2D data: use heatmap for Ising model or other spatial data
            # If 3D with last dimension of size 1, squeeze it
            ...
            figure = px.imshow(
                ...
                color_continuous_scale="RdBu_r",  # Red-Blue colormap (good for spin ±1)
            ...
            figure.update_layout(
                coloraxis_colorbar=dict(
                    title="Spin",
                    tickvals=[-1, 0, 1] if data.min() < 0 else [0, 0.5, 1],
                    ticktext=["-1", "0", "+1"] if data.min() < 0 else ["↓", "~", "↑"],
                )
            )
        else:
            # 1D data: use line plot for time series
            # Flatten if needed
            data_flat = data.flatten()
            figure = px.line(
                x=np.arange(len(data_flat)),
                y=data_flat,
                labels={"x": "t", "y": "amplitude"},
            )
    else:
        # Create empty graph - default to line plot
        figure = px.line(
            labels={"x": "t", "y": "amplitude"},
        )
