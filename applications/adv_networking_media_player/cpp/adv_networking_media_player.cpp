...
class MockReceiverOp : public holoscan::Operator {
 public:
  ...
  void initialize() override {
    HOLOSCAN_LOG_INFO("MockReceiverOp::initialize()");
    holoscan::Operator::initialize();

    port_id_ = ano::get_port_id(interface_name_.get());
    if (port_id_ == -1) {
  ...
  void compute(InputContext& op_input, OutputContext&, ExecutionContext& context) override {
    ...
    const auto num_rx_queues = ano::get_num_rx_queues(port_id_);
    for (int q = 0; q < num_rx_queues; q++) {
      auto status = ano::get_rx_burst(&burst, port_id_, q);
