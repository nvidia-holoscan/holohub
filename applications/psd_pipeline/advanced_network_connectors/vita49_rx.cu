...
using in_t = std::shared_ptr<holoscan::ops::AdvNetBurstParams>;
...
void Vita49ConnectorOpRx::setup(OperatorSpec& spec) {
  ...
  // to batch up 625x20x1024 packets.
  // We want to have 2 data buffers ping-ponging between processing
  // and accumulation, so we want to hold 2x625x20x1024 packets
...
void Vita49ConnectorOpRx::free_bufs_and_emit_arrays(
  ...
  auto data = channel->rf_data.Slice<2>({static_cast<index_t>(channel->cur_idx), 0, 0},
