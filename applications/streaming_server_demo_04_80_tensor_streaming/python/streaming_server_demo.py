...
class StreamingServerTestApp(Application):
    ...
    def __init__(
        ...
        server_name="HoloscanStreamingServer",
    ):
        """
    ...
    def compose(self):
        ...
        print("🚀 Streaming Server Application Configured:")
        print(f"   📡 Server: {self.server_name}")
        print(f"   🌐 Port: {self.port}")
        print(f"   📐 Resolution: {self.width}x{self.height}")
        print(f"   🎬 FPS: {self.fps}")
        print("   🔄 Mode: Echo Server (frames passed through without processing)")
        print("   📊 Pipeline: Upstream → Downstream (Echo Server - No Processing)")
...
- Sends frames back to clients (downstream)
    ...
    parser.add_argument(
        "--port", type=int, default=48010, help="Port for the streaming server (default: 48010)"
    )
    parser.add_argument(
        "--server-name",
        type=str,
    ...
    # Video parameters
    parser.add_argument(
        "--width", type=int, default=854, help="Frame width in pixels (default: 854)"
    )
    parser.add_argument(
        "--height", type=int, default=480, help="Frame height in pixels (default: 480)"
    )
    parser.add_argument(
