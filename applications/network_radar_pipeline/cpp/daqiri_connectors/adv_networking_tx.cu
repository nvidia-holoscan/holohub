...
void AdvConnectorOpTx::initialize() {
  ...
  register_converter<daqiri::NetworkConfig>();
  ...
  hds_ = (split_boundary_.get() && cfg_.get().ifs_[port_id_].tx_.queues_[0].common_.mrs_.size() > 1)
             ? PADDED_HDR_SIZE
             : 0;
  gpu_direct_ = false;
...
void AdvConnectorOpTx::compute(InputContext& op_input, OutputContext& op_output,
  ...
  if (rf_data == nullptr) {
    if (gpu_direct_ && out_q.size() > 0) {
      // If packet setup is done, send to DAQIRI
