...
class AdvConnectorOpTx : public Operator {
 ...
 private:
  ...
  static constexpr int MAX_DAQIRI_BATCHES = 10;  // DAQIRI bursts for one app batch
  static constexpr uint16_t queue_id   = 0;

  Status set_cpu_hdr(daqiri::BurstParams *msg, const int pkt_idx);
  ...
  Parameter<daqiri::NetworkConfig> cfg_;
  ...
  Parameter<std::string> interface_name_;  // Port name from DAQIRI config to poll on
