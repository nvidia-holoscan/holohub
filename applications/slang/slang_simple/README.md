...
> **Note:** The IGX Thor iGPU platform is not supported. The Slang compiler bundled with `SlangShaderOp`
> emits PTX that the CUDA driver on the IGX Thor iGPU platform cannot load, and the application fails at startup with
...
2. **`the provided PTX was compiled with an unsupported toolchain. (222)`**: The CUDA driver on the
   current GPU rejects the PTX generated by the bundled Slang compiler. This is a known limitation
   on the IGX Thor iGPU platform, which is not supported by this application. Run on a supported platform
   instead.
