...
def generate_smpte2110_file(
    file_path: str, width: int = 1920, height: int = 1080, bit_depth: int = 10, num_frames: int = 10
):
    """
    ...
    logger.info(
        f"Generating YCbCr 4:2:2 media file: {file_path} ({width}x{height}, {bit_depth}-bit, {num_frames} frames)"
    )
    ...
    logger.info(
        f"Frame size: {bytes_per_frame} bytes, Total file size: {total_size} bytes ({total_size / (1024*1024):.1f} MB)"
    )
