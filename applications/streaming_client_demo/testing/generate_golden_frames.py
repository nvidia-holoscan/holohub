...
from holoscan.operators import VideoStreamReplayerOp, FormatConverterOp, HolovizOp, VideoStreamRecorderOp
from holoscan.resources import UnboundedAllocator, CudaStreamPool, BlockMemoryPool
...
class StreamingClientTestAppWithRecording(Application):
    ...
    def compose(self):
        # Create allocators and resources
        allocator = UnboundedAllocator(self, name="allocator")
        cuda_stream_pool = CudaStreamPool(
            self, name="cuda_stream_pool", 
...
def main():
    parser = ArgumentParser(description="Generate golden reference frames for streaming_client_demo testing")
    parser.add_argument("--data", default=None, help="Path to video data directory")
    parser.add_argument("--output-dir", default="./recording_output", help="Output directory for recorded frames")
    parser.add_argument("--basename", default="streaming_client_demo_output", help="Basename for recorded files")
