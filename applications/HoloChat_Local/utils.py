...
def get_source_chunks(all_contents, file_type=None, chunk_size=1500, chunk_overlap=150):
    ...
    if file_type in ['python', 'cpp', 'markdown']:
        splitter = RecursiveCharacterTextSplitter.from_language(language=file_type, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
    else:
...
def get_source_chunks_lang(all_contents):
    encoder_name = 'p50k_base'
    tokenizer = tiktoken.get_encoding(encoder_name)
    print("Turning text into chunks ...")
    source_chunks = []
    for source in tqdm(all_contents, desc='Processing files...'):
        for chunk in tokenizer.encode(source.page_content, disallowed_special=()):
            source_chunks.append(Document(page_content=chunk, metadata=source.metadata))
    return source_chunks