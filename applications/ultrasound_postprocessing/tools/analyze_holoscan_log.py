...
def analyze_text_log(filename, discard=0):
    ...
    print("\n--- Performance Report ---")
    print(f"{'Operator':<25} | {'Avg (ms)':<10} | {'Min (ms)':<10} | {'Max (ms)':<10} | {'Count':<5}")
    print("-" * 75)
    ...
    if e2e_latencies:
        ...
        print("-" * 75)
        print(f"{'End-to-End':<25} | {mean(e2e_latencies):<10.3f} | {min(e2e_latencies):<10.3f} | {max(e2e_latencies):<10.3f} | {len(e2e_latencies):<5}")


def main():
    parser = argparse.ArgumentParser(description="Analyze Holoscan Tracker Log")
    parser.add_argument("logfile", help="Path to the log file")
    parser.add_argument("--discard", type=int, default=0, help="Number of samples to discard from start and end")
