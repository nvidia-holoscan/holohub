...
class AdvNetworkingBenchDocaTxOp : public Operator {
 public:
    ...
    if (gpu_direct_.get()) {
      ...
      if ((ip_dst_ & 0xff) == 2) {
        ...
        uint8_t payload[] = {
            ...
            0x2D, 0xD9, 0x30, 0xA1, 0x08, 0x00, 0x45, 0x00,
            0x1F, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11,
            0x00, 0x00, 0xC0, 0xA8, 0x02, 0x1B, 0xC0, 0xA8,
            0x02, 0x1C, 0x10, 0x00, 0x10, 0x00, 0x1F, 0x8E,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  void setup(OperatorSpec& spec) override {
    ...
    spec.param<bool>(gpu_comms_, "gpu_comms", "GPU Communications enabled",
        "Enable GPU communications", false);
  }
    ...
    */
    ...
    if (!first_time) {
      ...
      if ((ret_cuda != cudaSuccess)) {
        HOLOSCAN_LOG_ERROR("Falling behind on TX processing for index {} ret {}!", cur_idx, ret_cuda);
        return;
      }
    }
    ...
    copy_headers(gpu_bufs[cur_idx], pkt_header_,
    ...
    populate_packets(gpu_bufs[cur_idx], payload_size_.get(),
        adv_net_get_num_pkts(msg), header_size_.get(), streams_[cur_idx]);
